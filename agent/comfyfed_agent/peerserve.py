...
short-lived, platform-signed Grant (see `cloud/src/core/peer.ts` for how the
...
2. Auth header `X-ComfyFed-Grant: base64(json)` -- the JSON is the grant
   ...
   own fields (see the `POST /api/agent/peer-grant` response shape in
   `cloud/src/routes/peer.ts`,
...
# Ordered exactly as the server's GRANT_FIELDS (cloud/src/lib/signing.ts) / the signed payload
...
def verify_grant_signature(grant: dict, sig: str, platform_pubkey_hex: str) -> bool:
    ...
    malformed hex/signature just reads as "not valid" (fail-closed; the
    payload it checks is the one `cloud/src/core/peer.ts`'s `signGrant`
    signs)."""
    try:
