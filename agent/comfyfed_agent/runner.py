...
from . import (
    comfy, control, detect, fetcher, hardware, mps_compat, natmap, peerserve, remote_update,
    signing, update, whitelist,
)
...
# instead of 4401 for that case; see the close codes in cloud/src/do/hub.ts.
...
class PlatformConnection:
    ...
    async def send_hello(
        ...
        message = {
            "type": "hello",
            # 2026-09-23: the availability this worker has RIGHT NOW. A
            # reconnect used to be recorded as idle until the first heartbeat
            # (a full interval later), and the platform would dispatch a
            # second job to a Mac that was still rendering the first one.
            "state": state,
            "job_id": job_id,
            ...
            # the unknown fields (see the hello handler in cloud/src/do/hub.ts).
            "protocol": 5,
            ...
            # `grantTtlSeconds` in cloud/src/core/peer.ts). No
    ...
    async def send_heartbeat(
        ...
        # the platform's `"stage": "fetching_models"|absent` contract in
        # cloud/src/do/hub.ts, as
    ...
    async def send_update_ack(self, status: str, detail: str) -> None:
        """Answer a platform `update_agent` (spec §2.2); `status` is one of
        `remote_update.ACK_STATUSES`."""
        await self._send({"type": "update_ack", "status": status, "detail": detail})
...
class AgentLoop:
    ...
    def __init__(self, config: AgentConfig, cfg_path: str, connection_factory=PlatformConnection):
        ...
        # Remote update (spec §2.2). `exit_code` is what `main._cmd_run`
        # hands to `sys.exit` once the event loop is gone: None for every
        # ordinary exit, `update.RESTART_EXIT_CODE` after a successful
        # platform-initiated update (the supervisor restarts on it). It is
        # set here, on the event loop, because a SystemExit raised inside
        # `apply_update`'s worker thread would only kill that thread.
        self.exit_code: Optional[int] = None
        self._remote_updater = remote_update.RemoteUpdater(
            config, __version__, self._running_job_id, self._request_restart_for_update
        )
        # The in-flight `update_agent` handler (or deferred apply), kept off
        # the receive loop so a slow download never delays heartbeats.
    ...
    def _aggregate_state(
        ...
        if running_job_id is not None:
            return "busy", running_job_id, None
    ...
    def _running_job_id(self) -> Optional[str]:
        """The id of the job this PROCESS is executing right now (any
        platform), or None -- the single source for "is the agent busy"."""
        ...
        async with self.job_lock:
            ...
            try:
                ...
                if is_model_fetch or (fetch_models and self.config.auto_fetch_models):
                    ...
                    # cancelled receipt and a panel `job_running` push for a job
    ...
    def _on_job_task_done(self, task: asyncio.Task) -> None:
        ...
        # Success, failure and cancel all land here, so this is the one
        # place a `deferred` remote update gets its turn (spec §2.2).
        self._spawn_deferred_update_apply()
        if task.cancelled():
            return
        exc = task.exception()
        if exc is not None:
    ...
    def _spawn_update_task(self, coro, name: str) -> asyncio.Task:
        task = asyncio.get_running_loop().create_task(coro, name=name)
        self._update_task = task
        task.add_done_callback(self._on_update_task_done)
        return task

    def _spawn_deferred_update_apply(self) -> None:
        if self._remote_updater.pending is None:
            return
        if self._shutdown_in_progress:
            # The job ended because the operator is stopping the agent
            # (`comfyfed stop` / Ctrl-C): installing now would turn their
            # stop into a restart, and the loop is about to go away under
            # the pip thread anyway. Dropped, not parked: next start updates.
            logger.info(
                "停止流程中，略過延後的更新（下次啟動會自動更新） / shutting down; "
                "skipping the deferred update (the next start applies it)"
            )
            return
        self._spawn_update_task(
            self._remote_updater.apply_pending_if_idle(), name="comfyfed-deferred-update"
        )
    ...
    def _on_update_task_done(self, task: asyncio.Task) -> None:
        ...
        exc = task.exception()
        if exc is not None:
            logger.error("runner: remote update task %s raised", task.get_name(), exc_info=exc)

    def _request_restart_for_update(self) -> None:
        """Runs on the event loop after `apply_update` succeeded: record the
        restart exit code and start the same graceful wind-down Ctrl-C and
        `comfyfed stop` use (`_graceful_shutdown_and_stop` -> `loop.stop()`),
        after which `main._cmd_run` exits with `exit_code`."""
        self.exit_code = update.RESTART_EXIT_CODE
        if self._shutdown_in_progress:
            logger.info(
                "已在停止流程中，更新後的重啟交由現行關機流程 / already shutting down; "
                "the post-update restart rides on the wind-down in progress"
            )
            return
        self._shutdown_in_progress = True
        logger.info(
            "更新已安裝，正在停止工作並以 exit %d 結束以便重新啟動 / update installed; "
            "winding down and exiting %d so the supervisor restarts the agent",
            update.RESTART_EXIT_CODE, update.RESTART_EXIT_CODE,
        )
        loop = asyncio.get_running_loop()
        loop.create_task(self._graceful_shutdown_and_stop(loop), name="comfyfed-update-shutdown")
    ...
    async def _handle_message(self, conn: PlatformConnection, message: dict) -> None:
        ...
        if msg_type == "job":
            ...
            # A task, like jobs: the check and the wheel install block for
            # seconds, and this connection must keep heartbeating meanwhile.
            self._spawn_update_task(
                self._remote_updater.handle_update_agent(conn), name="comfyfed-remote-update"
            )
        else:
    ...
    async def _run_platform(self, conn: PlatformConnection) -> None:
        ...
        while True:
            try:
                ...
                # 2026-09-23: a Mac may take model jobs only when the RUNNING
                # ComfyUI carries the fp8/int8 MPS shim (mps_compat.py). File
                # I/O + a pid probe, so off the loop like the calls around it.
                hw[mps_compat.HARDWARE_KEY] = await asyncio.to_thread(
                    mps_compat.report, self.config.models_dir
                )
                ...
                await conn.send_hello(
                    ...
                    state=self._effective_state(conn.state),
                    job_id=self._job_id_for(conn),
                )
