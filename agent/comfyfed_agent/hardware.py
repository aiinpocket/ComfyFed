...
def collect_hardware(comfy_url: str) -> dict:
    ...
    try:
        ...
        pass
    if gpu_name is None and platform.system() == "Darwin":
        # 2026-09-23: Apple silicon has no separate VRAM -- the GPU shares the
        # whole unified pool, so report that as `vram_gb` and flag it, so the
        # platform gates the job's RESIDENT model set against one pool
        # (assess.estimate_resident_gb) instead of skipping the check.
...
def collect_dynamic(model_dir_or_none: str | None) -> dict:
    ...
    try:
        ...
        pass
    if free_vram_gb is None and platform.system() == "Darwin":
        # Unified memory: free "VRAM" is free RAM (see collect_hardware).
        free_vram_gb = free_ram_gb
...
def scan_models(models_dir: str, hash_models: bool = True) -> list[dict]:
    ...
    `matchesModelName` in `cloud/src/core/assess.ts`, which strips the leading
