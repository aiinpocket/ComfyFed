...
class Client:
    ...
    def _send(self, method: str, path: str, **kw: Any) -> httpx.Response:
        """傳輸層錯誤（逾時、連不上、DNS）→ `ToolError`，不要讓 httpx 的
        traceback 直接噴回 AI 客戶端（live-caught：平台的 HEAD 探測慢，
        `request_model` 超過 30 s 就整段 stack 吐出來）。"""
        try:
            return self._http.request(method, path, **kw)
        except httpx.TimeoutException as exc:
            raise ToolError(
                f"timeout: 平台 {TIMEOUT_SECONDS}s 內沒有回應 / no response within "
                f"{TIMEOUT_SECONDS}s ({method} {path})"
            ) from exc
        except httpx.HTTPError as exc:
            raise ToolError(f"transport: {type(exc).__name__}: {exc}") from exc

    def get(self, path: str, **kw: Any) -> Any:
        return self._json(self._send("GET", path, **kw))

    def post_json(self, path: str, body: Any) -> Any:
        return self._json(self._send("POST", path, json=body))
    ...
    def post_form(self, path: str, data: Mapping[str, str], files: Any = None) -> Any:
        ...
        if files is None:
            ...
            data = {}
        return self._json(self._send("POST", path, data=dict(data), files=files))
    ...
    def download(self, path: str, dest: Path) -> Path:
        ...
        try:
            ...
            raise ToolError(f"timeout: 下載沒有在 {TIMEOUT_SECONDS}s 內回應 / download timed out ({path})") from exc
        except httpx.HTTPError as exc:
            raise ToolError(f"transport: {type(exc).__name__}: {exc}") from exc
        return dest
...
def run_recipe(c: Client, recipe_id: str, params: dict, label: Optional[str] = None) -> Any:
    """用一個配方送單。平台會驗參數並渲染 workflow，回 `{job_id, recipe_id, params}`。
    `label`（選填，≤64 字）是工作名稱：console 任務列表會顯示，也是「檔案」頁
    產出成品的第一層資料夾；沒給時平台用配方 id。"""
    body: dict = {"params": params}
    if label is not None:
        body["label"] = label
    return c.post_json(f"/api/recipes/{recipe_id}/run", body)
...
def submit_workflow(
    ...
    `label` 同 `run_recipe`；沒給時平台從第一個 `Save*` 節點的 `filename_prefix` 推。"""
...
def build_server(settings: McpSettings, client: Optional[Client] = None):
    ...
    @server.tool()
    def run_recipe(recipe_id: str, params: dict, label: Optional[str] = None) -> dict:
        """用配方送單（平台驗參數並渲染 workflow）。`label` 是工作名稱（≤64 字），
        會成為檔案頁的資料夾名稱；沒給＝配方 id。
        Submit a job from a recipe. `label` names the job and becomes the
        folder name on the Files page (defaults to the recipe id)."""
        return impl["run_recipe"](c, recipe_id, params, label)

    @server.tool()
    def submit_workflow(
        workflow_json: str, requirements: Optional[dict] = None, label: Optional[str] = None
    ) -> dict:
        """送一份完整 ComfyUI API-format workflow JSON（配方不夠用時）。`label` 是
        工作名稱（≤64 字），會成為檔案頁的資料夾名稱；沒給＝第一個 Save 節點的 filename_prefix。
        Submit a raw ComfyUI API-format workflow when no recipe fits. `label`
        names the job / Files-page folder (defaults to the first Save node's
        filename_prefix)."""
        return impl["submit_workflow"](c, workflow_json, requirements, label)
