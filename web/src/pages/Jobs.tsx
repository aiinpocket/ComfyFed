...
export function Jobs({ role }: JobsProps) {
  ...
  const loadAll = useCallback(async () => {
    ...
    // 2026-09-21 分頁：只抓目前這一頁（最新在前），輪詢也只重抓這一頁。
    const [page, workers] = await Promise.all([
      api.listJobsPage(pageRef.current),
  ...
  // Page number lives in a ref as well as state so the polling loader (whose
  // identity `usePolling` deliberately freezes) always reads the current page
  // without restarting the interval. Changing page refetches immediately.
  ...
  const jobs = data?.page.jobs ?? [];
  const total = data?.page.total ?? 0;
  const limit = data?.page.limit ?? PAGE_SIZE;
        ...
        {pageCount > 1 && (
          <Group justify="space-between" wrap="wrap" gap="xs">
            <Text size="xs" c="dimmed">
              {t('jobs.page_summary', { page, pages: pageCount, total })}
            </Text>
            <Pagination value={page} onChange={changePage} total={pageCount} size="sm" withEdges />
          </Group>
        )}
      </Stack>
    </Stack>
...
function JobsTable({
  ...
  // Server order: newest first (`GET /api/jobs?page=`).
  const ordered = jobs;
              ...
              <Fragment key={job.id}>
                ...
                >
                  ...
                  <Table.Td>
                    ...
                    {/* 檔案頁 §2: the job's name, when it has one -- the same
                        string the Files page uses as a folder. */}
                    {job.label && (
                      <Text size="xs" c="dimmed" truncate="end" title={job.label} maw={160}>
                        {job.label}
                      </Text>
                    )}
                  </Table.Td>
