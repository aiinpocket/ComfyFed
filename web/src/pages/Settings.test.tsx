...
/**
 * The Settings page, exercised against a mocked `fetch` (the same seam
 * `api.ts` uses).
 *
 * The staging FILE LIST and its delete flow moved to `/files` on 2026-09-20
 * (檔案頁 §4) -- those cases now live in `Files.test.tsx`. What is left here
 * of `GET /api/staging` is the storage-quota bar, which still reads the same
 * listing's byte totals.
 */
...
describe('Settings page: admin upload limits', () => {
  ...
  it('hides the limits card from a non-admin', async () => {
    ...
    // The uploads card (quota only, now) still renders for every role.
    await screen.findByText('My uploads');
...
describe('Settings page: uploads quota line', () => {
  ...
  it('counts job inputs/outputs in the used-of-quota bar (2026-09-24)', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url === '/api/staging') {
        return jsonResponse({
    ...
    // 2 KB staging + 1 KB userdata + 4 KB job bytes.
    expect(await screen.findByText('Used 7 KB of 10 MB')).toBeInTheDocument();
  });
...
describe('Settings page: NSFW review key (2026-09-20)', () => {
  afterEach(() => {
    cleanup();
    vi.unstubAllGlobals();
  });

  it('reports whether a key is stored and POSTs the pasted key without ever showing it', async () => {
    const posts: unknown[] = [];
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === 'string' ? input : input.toString();
      const method = init?.method ?? 'GET';
      if (url === '/api/settings' && method === 'GET') {
        return jsonResponse({ platform_url: '', lang: 'en', object_info_mode: 'union', upload_max_file_mb: 50, upload_user_quota_gb: 5, split_batches: true, nsfw_check_api_key_set: false });
      }
      if (url === '/api/settings' && method === 'POST') {
        posts.push(JSON.parse(String(init?.body)));
        return jsonResponse({ platform_url: '', lang: 'en', object_info_mode: 'union', upload_max_file_mb: 50, upload_user_quota_gb: 5, split_batches: true, nsfw_check_api_key_set: true });
      }
      if (url === '/api/staging') return jsonResponse({ files: [], total_bytes: 0, quota_bytes: 0, userdata_bytes: 0 });
      if (url === '/api/auth/tokens') return jsonResponse([]);
    ...
    expect(posts[0]).toEqual({ nsfw_check_api_key: 'sk-ant-test' });
