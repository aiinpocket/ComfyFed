...
import { ReportRangeControl } from '../components/ReportRangeControl';
import { formatBytes, formatGpuSeconds, shortId } from '../lib/format';
import { DEFAULT_RANGE_VALUE, rangeParams, resolveRange, type RangeValue } from '../lib/reportRange';

/** Query params for the current preset/custom pick, resolved against "now"
 * at call time so relative presets always mean the trailing window. */
function paramsFor(range: RangeValue): { from?: string; to?: string } {
  return rangeParams(resolveRange(range));
}
...
function ContributionsPanel() {
  ...
  const [range, setRange] = useState<RangeValue>(DEFAULT_RANGE_VALUE);
