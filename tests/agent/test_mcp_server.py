...
def test_submit_workflow_without_requirements_omits_field():
    ...
    body = fake.last.content.decode("utf-8")
    assert 'name="requirements"' not in body
    assert 'name="label"' not in body


def test_run_recipe_forwards_label_only_when_given():
    fake = FakePlatform()
    fake.json_route("POST", "/api/recipes/flux-t2i/run", {"job_id": "j1"}, status=201)

    mcp_server.run_recipe(fake.client(), "flux-t2i", {"prompt": "cat"}, label="貓咪系列")
    assert json.loads(fake.last.content) == {"params": {"prompt": "cat"}, "label": "貓咪系列"}

    mcp_server.run_recipe(fake.client(), "flux-t2i", {"prompt": "cat"})
    assert "label" not in json.loads(fake.last.content)


def test_submit_workflow_forwards_label_as_a_form_field():
    fake = FakePlatform()
    fake.json_route("POST", "/api/jobs", {"job_id": "j4"})

    mcp_server.submit_workflow(fake.client(), "{}", label="series-a")

    body = fake.last.content.decode("utf-8")
    assert 'name="label"' in body and "series-a" in body
...
# --------------------------------------------------------------------------
# transport errors → ToolError（live-caught：request_model 逾時噴 httpx traceback）
# --------------------------------------------------------------------------


def _raising_client(exc: Exception) -> mcp_server.Client:
    def handler(request):
        raise exc

    settings = mcp_server.McpSettings(platform_url="https://p.example", token="cft_x")
    return mcp_server.Client(settings, transport=httpx.MockTransport(handler))


def test_timeout_becomes_tool_error_not_traceback():
    c = _raising_client(httpx.ReadTimeout("slow"))
    with pytest.raises(mcp_server.ToolError) as ei:
        mcp_server.request_model(c, "m.safetensors", "diffusion_models", "https://huggingface.co/x/y")
    assert str(ei.value).startswith("timeout:")
    assert "cft_x" not in str(ei.value)


def test_connection_error_becomes_tool_error(tmp_path):
    c = _raising_client(httpx.ConnectError("refused"))
    with pytest.raises(mcp_server.ToolError) as ei:
        mcp_server.list_recipes(c)
    assert str(ei.value).startswith("transport:")
    with pytest.raises(mcp_server.ToolError):
        mcp_server.download_results(c, "j1", dest_dir=str(tmp_path))
