...
class FakeConnection:
    ...
    def __init__(self, entry: PlatformEntry, config: AgentConfig):
        ...
        # (status, detail) of every `update_ack` (spec §2.2 remote update).
        self.update_acks: list[tuple[str, str]] = []
    ...
    async def send_update_ack(self, status, detail):
        self.update_acks.append((status, detail))
...
async def test_update_agent_message_is_dispatched_to_the_remote_updater(two_platform_loop, monkeypatch):
    """`_handle_message` routes `update_agent` to the updater (and still warns,
    rather than crashes, on a type it does not know)."""
    conn_a = two_platform_loop.connections["worker-a"]
    handled: list[object] = []

    async def _fake_handle(conn):
        handled.append(conn)

    monkeypatch.setattr(two_platform_loop._remote_updater, "handle_update_agent", _fake_handle)

    await two_platform_loop._handle_message(conn_a, {"type": "update_agent"})
    await asyncio.wait_for(two_platform_loop._update_task, timeout=5)

    assert handled == [conn_a]


async def test_unknown_message_type_only_logs_a_warning(two_platform_loop, caplog):
    conn_a = two_platform_loop.connections["worker-a"]
    with caplog.at_level(logging.WARNING, logger="comfyfed_agent.runner"):
        await two_platform_loop._handle_message(conn_a, {"type": "no_such_thing"})
    assert "unknown message type 'no_such_thing'" in caplog.text
...
@pytest.mark.asyncio
async def test_send_hello_carries_the_current_state_and_job_id():
    """2026-09-23: a reconnecting agent must not be recorded idle while it is
    still rendering -- the hello says busy + which job."""
    entry = PlatformEntry(
        platform_url="http://p", platform_pubkey="aa", worker_id="w1", certificate="cert", signing_key_hex="00" * 32,
    )
    conn = PlatformConnection(entry, AgentConfig())
    conn.ws = _RecordingWS()
    await conn.send_hello({}, "mps", "2.14", [], state="busy", job_id="job-1")
    payload = json.loads(conn.ws.sent[0])
    assert payload["state"] == "busy" and payload["job_id"] == "job-1"

    conn.ws = _RecordingWS()
    await conn.send_hello({}, "mps", "2.14", [])
    payload = json.loads(conn.ws.sent[0])
    assert payload["state"] == "idle" and payload["job_id"] is None
