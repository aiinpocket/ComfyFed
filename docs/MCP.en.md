...
**`nsfw_ok` describes the weights, not a permission.** The official Flux dev weights steer away from explicit content, so `flux-t2i` is flagged false — and that is why it is not the default. The two defaults use uncensored weights. The platform reads it in exactly one situation: once an admin turns a user's NSFW permission off on the console's Users page, that user's `run_recipe` of an `nsfw_ok: true` recipe answers `nsfw_not_allowed` (contact an administrator to allow it). For everyone else it is your federation and your rules.
...
| `run_recipe` | Submit from a recipe → `{job_id, recipe_id, params, model_fetch_jobs}`; optional `label` (job name; defaults to the recipe id) |
| `submit_workflow` | Submit a raw ComfyUI API-format workflow (only when no recipe fits); optional `label` (defaults to the `filename_prefix` of the first `Save*` node) |
...
**`label` (job name)**: both `run_recipe` and `submit_workflow` accept a `label` of up to 64 characters. It shows in the console's job list and is the **top-level folder name** for the job's outputs on the console's Files page (`label / date /`). When a user runs several images on one theme, give them the same label and they land in one folder. Without it: a recipe run uses the recipe id, a raw workflow uses the first `Save*` node's `filename_prefix`, and failing both, the first 8 characters of the job id.
