...
- `ineligible`：附上白話原因，例如缺少節點類別、VRAM 不足；模型類的 `ineligible` 現在只會出現在平台**完全不認得**這個模型（不在 curated 清單、也沒人回報過共識雜湊、無可信雜湊可簽）的時候。
