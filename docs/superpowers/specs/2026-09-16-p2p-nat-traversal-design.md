...
- 平台在收到 hello 且 `peer_url` 存在時，非同步對 `peer_url + "/peer/health"` 發 GET（connect+read 3 秒），結果寫入 `peer_reachable`／`peer_checked_at`。Python 用 `httpx` 於執行緒。**（2026-09-16 上線後修訂）** Cloud **不能用 `fetch`**：Workers 的子請求「只能打 URL，不能直接打 IP 位址」（Cloudflare Workers known issues），而本節刻意只探 IP 字面值，用 `fetch` 會讓每台種子都判成不可連（實機驗證：UPnP 開埠成功、外部埠檢服務確認開著，雲端仍寫 0）。改用 `cloudflare:sockets` 的 `connect()` 走 raw TCP 送最小 HTTP/1.1 GET，只讀狀態列（最多 512 bytes）、只認 204、不看 Location、3 秒逾時；仍在免費額度內。
