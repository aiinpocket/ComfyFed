...
- zh-TW-first bilingual notes matching existing template voice; suite green at every commit (`.venv/Scripts/python.exe -m pytest tests -q`, base 552; known flake tests/server/test_auth.py::test_read_session_payload_rejects_absent_and_tampered_cookies — rerun, don't chase) (resolved in Phase 1.9 Task 8 — root cause: the tamper flipped the token's last base64 char, whose bits are partly discarded padding, so ~6% of runs produced an alias that still decoded to the same signature).
