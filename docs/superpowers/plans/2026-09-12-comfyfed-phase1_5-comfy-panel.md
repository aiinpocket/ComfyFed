...
- 範本三支（皆為實戰驗證過的圖）：①wuxia-t2i（flux 文生圖）②character-portrait（flux 角色立繪）③ref2v-video（MiniMax H3 參考圖生影片，源自 make_film3.py 的 ref 模式圖）。每支含 Note/MarkdownNote 便條與群組框，逐步雙語說明「這個節點在做什麼、novice 該改哪裡」。
