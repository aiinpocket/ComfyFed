...
**`nsfw_ok` 是「模型本身會不會迴避露骨內容」的旗標，不是授權。** 官方 Flux dev 權重會迴避，所以 `flux-t2i` 標 false，也因此**不是**預設；預設的 `chroma-t2i` 與 `h3-t2v` 用的是不審查的權重。平台只在一種情況看它：管理員在 Console「使用者」頁把某人的 NSFW 權限關掉後，那個人用 `run_recipe` 跑 `nsfw_ok: true` 的配方會收到 `nsfw_not_allowed`（請聯絡管理員放行）；其他人的聯邦、其他人的規矩。
...
| `run_recipe` | 用配方送單 → `{job_id, recipe_id, params, model_fetch_jobs}`；選填 `label`（工作名稱，沒給＝配方 id） |
| `submit_workflow` | 送完整的 ComfyUI API-format workflow JSON（配方不夠用時才用）；選填 `label`（沒給＝從 `SaveImage`／`SaveVideo` 的 `filename_prefix` 推） |
...
**`label`（工作名稱）**：`run_recipe` 與 `submit_workflow` 都可以帶一個最多 64 字的 `label`。它會出現在 console 的任務列表，而且是 console「檔案」頁裡產出成品的**第一層資料夾名稱**（`label / 日期 /`）。使用者用同一個主題連跑幾張圖時，給同一個 label 就會歸在同一個資料夾。沒給時：配方送單用配方 id，raw workflow 用第一個 `Save*` 節點的 `filename_prefix`，都沒有就用 job id 前 8 碼。
