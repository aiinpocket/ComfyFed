...
**A dead registration is moved to `agent.dead.json` automatically.** When a platform rejects a registration with 4401 (the worker was deleted, or its credentials are invalid), the agent stops retrying it *and* takes that `platforms` entry out of `agent.json`, appending a full backup to `agent.dead.json` in the same directory (a JSON list that grows, with `removed_at` and `reason` added). The next start no longer attempts it or re-logs the error. **That backup contains a signing key in the clear — guard it exactly like `agent.json`.** To restore one, copy the record's `platform_url`, `platform_pubkey`, `worker_id`, `certificate` and `signing_key_hex` back into `agent.json`'s `platforms` list (leave `removed_at`/`reason` behind) and restart the agent. Normally, just re-run the installer to register afresh.
