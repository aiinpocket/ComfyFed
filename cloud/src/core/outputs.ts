 ...
 * ports the former Python server's `job_outputs` /
...
// the former Python `_MEDIA_OUTPUT_NODE_CLASSES` / `_TEXT_OUTPUT_NODE_CLASS` /
 ...
 * output node classes, so artifacts stay reachable. Ports the former Python
 ...
 * the former Python panel-WS `job_outputs(job) or {FALLBACK_OUTPUT_KEY: {}}`). */
 ...
 * preview. Ports the former Python `_read_text_artifact`. Capped at
 ...
 * ports the former Python `_merge_output`. A plain assignment would let two
 ...
 * job's outputs. Ports the former Python `job_outputs` exactly (the
 * split-by-extension / fallback-key / PreviewAny-duplication contract);
...
export async function jobOutputs(job: JobOutputsInput, store: R2Bucket): Promise<Record<string, OutputPayload>> {
  ...
  if (textEntries.length > 0) {
    ...
    // the former Python server's documented (not "fixed") one-SaveText-per-workflow
    // contract.
