 ...
 * from the former Python server (2026-09); this file is now the only
 * implementation.
 *
 ...
 *
 * 1. `SOURCES` -- the same twelve models curated by hand in the Python
...
/** The compute backend every model is assumed to run on unless its guide
 * entry says otherwise -- ports `model_guide.DEFAULT_MODEL_BACKENDS`. */
export const DEFAULT_MODEL_BACKENDS: readonly string[] = ["cuda"];
...
export interface ModelSource {
  ...
  gated: boolean;
  /** 2026-09-20: which worker backends this model is KNOWN to run on;
   * omitted = NVIDIA only (fp8/nvfp4 weights have no MPS kernels, bf16
   * image/video models are impractically slow there). Add "mps" only after
   * actually running the model on a Mac. Ports `ModelSource.backends`. */
  backends?: readonly string[];
  // Phase 3.2: operator-vouched trust anchor for the 12 curated entries
  ...
  // `model_manifest.ts`'s header comment and the Phase 3.2 addendum. `harvest()`-
...
// The twelve curated models -- copied verbatim from the former Python
// `SOURCES`. DO NOT reorder/rename/reword any field: byte parity with the
...
// federation's own LIVE `model_hashes` consensus. Byte-identical to the
...
export const SOURCES: Record<string, ModelSource> = {
  ...
  // Entry #12, 2026-09-20 -- the Heretic-abliterated build of the same
  // Qwen3-VL-4B-Instruct as `qwen3vl_4b_bf16` above, which the two
  // prompt-helper templates moved to because the aligned one refuses an
  // explicit prompt request. `backupUrl: null`: nothing has been uploaded to
  // `comfyfed-models` for it, and a mirror URL that 404s is worse than none
  // (the rule Chroma1-HD established).
  "qwen3-vl-4b-heretic.safetensors": {
    name: "qwen3-vl-4b-heretic.safetensors",
    directory: "text_encoders",
    sizeGb: 8.27,
    officialPage: "https://huggingface.co/DreamFast/Qwen3-VL-4b-Heretic-ComfyUI",
    officialUrl:
      "https://huggingface.co/DreamFast/Qwen3-VL-4b-Heretic-ComfyUI/resolve/main/qwen3-vl-4b-heretic.safetensors",
    backupUrl: null,
    gated: false,
    sha256: "3fba9f5a2059c719da105bcb510b9df0450bddde52d96968c607802c9d86f9ce",
    sizeBytes: 8875713408,
  },
...
// `cloud/scripts/seed-official.mjs` uploads there. If Task 10 lands a different prefix, update
 ...
 * ports the former Python `_model_entries`. Fully recursive: per-node
...
/** Backends `name` is known to run on -- ports `model_guide.model_backends`:
 * the curated entry's `backends`, else `DEFAULT_MODEL_BACKENDS`. Static
 * (no harvest): the official-template harvest carries no backend knowledge. */
export function modelBackends(name: string): ReadonlySet<string> {
  const source = SOURCES[name];
  return new Set(source?.backends ?? DEFAULT_MODEL_BACKENDS);
}
 ...
 * each `node_errors[...].errors[].message` -- deliberately short. */
