/**
 * 平台端的種子可連性驗證（Phase 3.4 §4.2）。自原 Python 伺服器移植
 * （2026-09）；本檔現為唯一實作，與原版逐項對齊（同樣的私有網段清單、
 ...
 * 兩種形狀要處理。對應原 Python 版 `_ip_literal` 的回傳。 */
 ...
 * 自己走 `socket.inet_aton`（原 Python 版的 `_ip_literal`）。 */
...
function parseIpLiteral(host: string): IpLiteral | null {
  ...
  if (v6 !== null) {
    ...
    // 否則它不落在任何一條 IPv6 私有網段裡。Parity: 原 Python 版用
...
/** 拒絕清單（parity: 原 Python 版的 `PRIVATE_NETWORKS`）。spec §4.2 逐字：
 ...
 * Ports the former Python `peer_host_address`. */
 ...
 * 被探（見 `isIpLiteralPeerUrl`）。Ports the former Python
 * `is_private_peer_url`. */
 ...
 * （fix round 1 裁示）。Ports the former Python `is_ip_literal_peer_url`. */
 ...
 * Ports the former Python `health_url`. */
 ...
 * 就收線，狀態列一到手立刻取消讀取（parity: 原 Python 版的
 ...
 *
 * Ports the former Python `refresh`. */
...
/** 心跳時是否該重測。Ports the former Python `needs_recheck`. */
