/**
 * Pure login-backoff logic, ported from the former Python server (2026-09;
 * its `_consecutive_failures` / `_required_wait_seconds`). This file is now
 * the only implementation. Kept dependency-free
 ...
 * of the most recent failure, mirroring the former Python `_consecutive_failures`.
 *
...
// -- Error messages, copied verbatim from the former Python server (English
// there, not zh-TW -- only the settings.bad_object_info_mode message is zh-TW
// in the Python source; everything else there is plain English) -----------
...
export const MESSAGES = {
  ...
  // Mirrors the former Python `require_admin`: "403 for a logged-in non-admin."
...
// -- Users API messages (Phase 3.0), copied verbatim from the former Python
// `users` module ----------------------------------------------------------
 ...
 * (`('admin', 'user')`), since the former Python `_validate_role` error message
