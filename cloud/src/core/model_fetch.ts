 ...
 * implementation. The trust model: a worker
 ...
 * wiring in `do/hub.ts` (the same split the former Python server had).
 */
 ...
 * (final-review I4). Mirrors the former Python `_HEAD_MAX_REDIRECTS`. */
 ...
 * `TRUSTED_ORIGINS` -- ports the former Python `is_trusted_url`.
 *
 ...
 * Mirrors the former Python `_INTERNAL_HOST_SUFFIXES`. */
 ...
 * ports the former Python `is_safe_redirect_target`.
 *
 ...
 * the former Python `head_size_bytes`.
 *
 ...
 * the former Python `unverified_payload`.
 *
 ...
 * the former Python `sign_unverified_entry`. Uses the same Ed25519 helper
 ...
 * the former Python `is_unverified_entry`. */
...
/** §5.1's refusal messages, copied verbatim from the former Python `_MESSAGES`
...
/** One of §5.1's refusal rows -- ports the former Python `FetchRequestError`.
 ...
 * ports the former Python `_safe_relative`. An empty directory is fine (the
...
/** A bare model filename -- ports the former Python `_safe_name`: no path
 ...
 * holds `name` (§5.1 row 2); ports the former Python `_fleet_has_model`.
...
/** §5.1 row 7's "message 列出原因" -- ports the former Python
 ...
 * the former Python `create_fetch_job`. Returns `{jobId, reused}`; throws
 ...
 * job (the route turns that into a 404) -- ports the former Python
 * `fetch_status`.
 *
