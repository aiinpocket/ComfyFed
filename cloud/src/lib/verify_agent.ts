/**
 * Ed25519-signed agent request verification, ported from
 * `server/comfyfed_server/workers.py`'s `verify_agent` (+ `_canonical_message`
...
/** Matches `workers.py`'s `_NONCE_TTL_SECONDS` / `_MAX_TS_SKEW_SECONDS`. */
...
export async function verifyAgentRequest(
  ...
  // for a merely-guessed worker id (see workers.py's comment on this exact
  // point).
