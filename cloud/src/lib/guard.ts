/**
 * Hono middleware/helpers for session auth + CSRF, mirroring the former
 * Python server's `require_user` / `require_admin` /
 ...
 * the former Python server's `_http_exception_handler` shape
 * (`r.json()["error"]["code"]`). */
 ...
 * absent/invalid/expired. Public (like the former Python `read_session_payload`)
 ...
 * `requireAdmin` middleware) for that, exactly like the former Python server's
...
export async function readSession(c: Context<{ Bindings: Env }>): Promise<SessionPayload | null> {
  ...
  // Parity with the former Python `read_session_payload`: `if not session_cookie:
 ...
 * the token-management / change-password / logout routes. Mirrors the former Python
 ...
 * Mirrors the former Python `_session_user_from_payload`: rejects (returns `null`) a
 ...
 * mirrors the former Python `resolve_session_user`, used by the same three
 ...
 * mirrors the former Python `_bearer_user`, except that it returns `null` on failure
 ...
 * `requireCsrf`/`requireCsrfUser` -- mirrors the head of the former Python
 * `require_user`.
 *
...
/** Any logged-in, enabled, current-epoch user -- mirrors the former Python
...
/** `requireUser` plus `role === 'admin'` -- mirrors the former Python
...
/** Admin gate + CSRF -- mirrors the former Python `require_csrf`: 401 as
 ...
 * mirrors the former Python `require_csrf_user`. Phase 3.0's `change-password` is
 ...
 * §4.3's exception list -- mirrors the former Python `require_session`, and guards
 * `GET /api/auth/tokens`.
 *
 ...
 * (spec §4.3's exception list) -- mirrors the former Python `require_csrf_session`.
 *
