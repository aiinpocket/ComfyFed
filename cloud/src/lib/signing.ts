 ...
 * confirmed against the former Python server (see task-1-report.md's
 * "Canonical-string finding"; the golden vectors are the fixed fixture
 * `cloud/test/fixtures/golden.json`):
 *
 *  1. receipt payload     (former Python `_sign_and_store_receipt`):
 *       f"{job_id}|{worker_id}|{gpu_seconds:.1f}"
 *  2. registration cert   (former Python `register()`):
 *       f"{worker_id}|{pubkey_hex}"
 *  3. release signature   (former Python `_publish_agent`):
 *       f"{version}|{sha256_hex}"
 *  4. signed-request canonical message (former Python `_canonical_message`):
 *       f"{METHOD}\n{path}[?{query}]\n{ts}\n{nonce}\n" + body
 *       -- the body is embedded RAW (not hashed) -- byte-for-byte.
 *  5. fetch-manifest entry (former Python `model_manifest.entries()`):
 *       f"{name}|{directory}|{sha256}|{size_bytes}"
 *  6. P2P grant (former Python `sign_grant`/`verify_grant`, Phase 3.1 Task 3/8):
 *       f"{grant_id}|{name}|{size_bytes}|{sha256}|{seeder_id}|{puller_id}|{expires_at}"
 *  7. P2P upload receipt (former Python `peer_served`'s dedicated signing string,
...
/** Ordered exactly as the former Python `_GRANT_FIELDS` / the Global Constraints
...
/** `f"{grant_id}|{name}|{size_bytes}|{sha256}|{seeder_id}|{puller_id}|{expires_at}"`
 * -- ports the former Python `_grant_payload` verbatim (field order = `GRANT_FIELDS`). */
export function buildGrantPayload(grant: GrantFields): string {
  return GRANT_FIELDS.map((field) => String(grant[field])).join("|");
}

/** `f"p2p_upload|{grant_id}|{worker_id}|{bytes_served}"` -- ports the former Python
