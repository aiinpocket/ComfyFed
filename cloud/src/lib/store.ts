/**
 * R2 artifact/input-asset storage helpers, ported from the former Python
 * server (2026-09); this file is now the only implementation. Phase 1's `LocalStore` wrote to
 ...
 *
 * `sanitizePathComponent` matches the former Python function of the same name in
 ...
 * separated) plus a manual backslash split, since the former Python
...
export function sanitizePathComponent(value: string, what = "path component"): string {
  ...
  // the former Python server's identical check: a NUL makes Python's `open()` throw deep
 ...
 * type to render the `jobs.bad_asset_name` 400, matching the former Python
 ...
 * the former Python `SHARED_STAGING_UID`) -- namespaced alongside real
...
/** The prefix EVERY user's staging sits under (`staging/`), for the admin
 * cross-user listing in `routes/staging.ts` -- the key after it is
 * `<uid>/<filename>`. */
export function stagingRootPrefix(): string {
 ...
 * throw `InvalidPathComponent`. Mirrors the former Python
 ...
 * of the former Python server's `<data_dir>/comfy_userdata/<uid>/` tree, holding the
