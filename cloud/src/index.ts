...
// `MODE` is "cloud" on Cloudflare and "selfhost" under scripts/selfhost.mjs
// (same Worker, different runtime) -- see docs/SELF-HOSTING.*.md.
app.get("/api/ping", (c) => c.json({ ok: true, mode: c.env.MODE || "cloud" }));

// Session gate for the embedded panel, ported from the former Python
...
// assets, matching the former Python server (its templates router had no
...
// `/comfy` (no trailing slash) -> `/comfy/`, mirroring the former Python
// `_comfy_root`: the pinned frontend derives its API base from
// `location.pathname`, so the trailing slash is load-bearing. Placed after the
...
// same reason the former Python `create_ws_router` did: the pinned frontend
