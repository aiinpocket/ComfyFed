 ...
 * issuance and bandwidth booking. Parity source: `server/comfyfed_server/
 * peer.py`'s `create_router`, read in full; byte-parity requirements (grant
 * signature format, response shape, error codes) are pinned in the plan's
 * Global Constraints and `peer.py`'s own docstrings.
 *
 ...
 * (`queries.claimP2pGrantBooked`), the D1-correct equivalent of peer.py's
...
app.post("/api/agent/peer-served", async (c) => {
  ...
  // the D1-correct equivalent of peer.py's lock-guarded check-then-set --
