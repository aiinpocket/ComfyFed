/**
 * `/api/jobs/*` and `/api/agent/jobs/*` -- ported from the former Python
 * server (2026-09; this file is now the only implementation), plus its
...
import type { FetchProgress, Job, Receipt } from "../db/queries";
...
import { artifactKind, deriveLabel, normalizeLabel } from "../core/label";
import { fileCapExceeded, readLimits, tooLargeMessage, usageBytes, quotaMessage, QUOTA_EXCEEDED_CODE } from "../lib/limits";
...
/** The `{path, query}` the agent signed, from the request URL. The path is
 * the DECODED form: the agent signs the raw path it builds
 * (`.../inputs/images (1).jpg` -- runner.py `_download_input`), httpx sends
 * it percent-encoded, and the Python server verifies against Starlette's
 * `request.url.path`, which is unquoted. `URL.pathname` is still encoded,
 * so before 2026-09-20 any job asset whose name had a space/paren 401'd on
 * cloud only. A malformed escape (`%zz`) falls back to the encoded form so
 * the signature check fails closed rather than the route throwing. The
 * query stays raw -- both stacks compare it as sent. */
function requestInfo(url: string): { path: string; query: string } {
  ...
  return { path, query: u.search ? u.search.slice(1) : "" };
}
...
// ---------------------------------------------------------------------------
// Job -> JSON dict shaping -- ports the former Python `_job_dict` / `_job_dict_full`
// / `_receipt_dict` exactly (field names/shape).

/** Ports the former Python `_job_dict`. `hub` (optional) enables the Phase 2.1
...
async function jobDict(job: Job, hub?: DurableObjectNamespace): Promise<Record<string, unknown>> {
  const d: Record<string, unknown> = {
    id: job.id,
    status: job.status,
    // 2026-09-20 檔案頁 §2：可為 null（舊列、推不出名稱的單、model_fetch
    // 單），前端自己用 job id 前 8 碼頂替。Ports the former Python `_job_dict`.
    label: job.label,
    origin: job.origin,
    ...
    // url、是否 unverified）。Ports the former Python `_job_dict`.
    ...
    // `queries.updateJobRequeuedForRetry`）。Ports the former Python `_job_dict`.
...
/** `[children, gpuSecondsTotal]` for a parent -- ports the former Python
...
// of the former Python `_require_owner_or_admin`) -- NOT the agent-facing artifact
...
// ports the former Python `upload_job_artifact` inline check (owns_it OR the blip
// re-adoption window).
...
// ---------------------------------------------------------------------------
// Submission-relaxation gate (Phase 2.1 Task 7) -- ports the former Python
 ...
 * it into the same `errorJson` envelope. Mirrors the former Python
 ...
 * one such path. Same extraction the former Python server made.
 *
...
export async function createJobFromWorkflow(
  ...
  opts: {
    ...
    label?: string | null;
    /** 2026-09-20 NSFW 閘：配方自己宣告的 `nsfw_ok: true`（只有 routes/recipes.ts
     * 會傳）。對一個被關掉 NSFW 的 user，這面旗子本身就是拒絕理由。 */
    nsfwOk?: boolean;
    /** `true` = 呼叫端**已經**跑過同一個閘（routes/recipes.ts 必須在排模型下載
     * 之前就擋），這裡不再問第二次。與 `fetching` 一樣是程式內部的具名選項，
     * 沒有任何請求欄位流得進來。 */
    nsfwChecked?: boolean;
  } = {}
): Promise<string> {
  ...
  // 2026-09-20 NSFW 閘：在做任何事之前擋（含檔名／大小檢查）。對 `nsfw_allowed`
  // 沒被關掉的 user（預設）這是一次 users 表讀取就回來。`classify` 走模組命名
  // 空間（見 core/nsfw_gate.ts 檔頭）。
  if (opts.nsfwChecked !== true) {
    const nsfwRefused = await nsfwGate.checkSubmission(env, user.uid, workflow, {
      recipeNsfwOk: opts.nsfwOk === true,
      classify: nsfwGate.classifyWithClaude,
    });
    if (nsfwRefused !== null) {
      throw new JobCreationError(403, nsfwGate.NSFW_NOT_ALLOWED_CODE, nsfwRefused);
    }
  }
  ...
  // behind.
  const uploadLimits = await readLimits(env.DB, user.uid);
  ...
  // 2026-09-24 配額納入 job 位元組：這些資產會落在 `job_inputs/<job_id>/`，
  // 從此算在送件者的配額裡（lib/limits.ts），所以整批加起來塞不下就在這裡
  // 擋，跟單檔上限一樣在寫 job 列之前。空的送件（沒有資產）不用問儲存層。
  // Same 413 + `quota_exceeded` every other upload surface answers with.
  // Ports the quota check inside the former Python `create_job_from_workflow`.
  const incomingBytes = assetFiles.reduce((sum, f) => sum + f.size, 0);
  if (incomingBytes > 0) {
    const used = await usageBytes(env.DB, env.STORE, user.uid);
    if (used + incomingBytes > uploadLimits.quotaBytes) {
      throw new JobCreationError(413, QUOTA_EXCEEDED_CODE, quotaMessage(used, uploadLimits));
    }
  }
  ...
  await queries.insertJob(env.DB, {
    ...
    userId: user.uid,
    // 2026-09-20 檔案頁 §2：呼叫端給的名稱優先，沒給（或全是空白）就從
    // workflow 的第一個 `Save*` 節點推。回退寫在這裡而不是每個入口，是為了
    // 讓 console、配方、面板對同一張圖得到同一個名稱，而不必各自記得要問。
    label: normalizeLabel(opts.label) ?? deriveLabel(workflow),
  ...
  // 2026-09-24：輸入落地後一次寫定 `input_bytes`（0 = 確定沒有輸入，與
  // 0017 之前那些 NULL 的舊單分得開）。
  await queries.setJobInputBytes(env.DB, jobId, incomingBytes);
...
app.post("/api/jobs", requireCsrfUser, async (c) => {
  ...
  // 2026-09-20 檔案頁 §2：選填的工作名稱。沒給（或給空白）就從 workflow 推，
  // 推不出來存 NULL -- 舊的送件端完全不需要改。
  const label = typeof form["label"] === "string" ? (form["label"] as string) : null;
...
/** 2026-09-21 分頁：`?page=`（1 起算）與 `?limit=`（預設 25，夾在 1..100）。
 * 沒帶 `page` 回 null = 不分頁。非數字或 < 1 的 page 當 1；limit 同理夾範圍，
 * 不 4xx —— 這只是列表的顯示參數，寬鬆讀比讓前端撞錯誤值得。 */
...
export function parsePaging(
  ...
  if (pageParam === undefined) return null;
  const pageN = Number.parseInt(pageParam, 10);
  const page = Number.isFinite(pageN) && pageN >= 1 ? pageN : 1;
  const limitN = limitParam === undefined ? PAGE_LIMIT_DEFAULT : Number.parseInt(limitParam, 10);
  const limit = Number.isFinite(limitN) ? Math.min(PAGE_LIMIT_MAX, Math.max(1, limitN)) : PAGE_LIMIT_DEFAULT;
  return { page, limit };
}

/** The transient `stage`/`fetch_pct`/`fetch_model` trio `jobDict` adds when a
 * job is mid model-fetch -- same field-presence rules as the per-job path
 * (each key only when non-null), lifted out so the paged list can merge a
 * batch lookup's entry instead of a per-row DO call. */
function fetchProgressFields(progress: FetchProgress | null): Record<string, unknown> {
  const d: Record<string, unknown> = {};
  if (!progress) return d;
  if (progress.stage !== null) d.stage = progress.stage;
  if (progress.fetch_pct !== null) d.fetch_pct = progress.fetch_pct;
  if (progress.fetch_model !== null) d.fetch_model = progress.fetch_model;
  return d;
...
app.get("/api/jobs", requireUser, async (c) => {
  ...
  // 2026-09-21 分頁：帶了 `?page=` 才走分頁信封（最新在前、附 total）；沒帶
  // 維持原本的整包陣列（舊到新）—— MCP 的 `list_jobs` 與既有測試都靠它。
  const paging = parsePaging(c.req.query("page"), c.req.query("limit"));
  if (paging) {
    const { jobs, total } = await queries.listJobsPage(c.env.DB, statuses, {
      ...(isAdmin ? {} : { userId: user.uid }),
      includeChildren,
      page: paging.page,
      limit: paging.limit,
    });
    // 一頁的 fetch 進度一次問完，不再每列各打一次 Hub DO。
    const progressById = await queries.getFetchProgressBatch(
      c.env.HUB,
      jobs.map((j) => j.id)
    );
    const usernameById = isAdmin
      ? await queries.getUsernamesByIds(
          c.env.DB,
          [...new Set(jobs.map((j) => j.userId).filter((id): id is string => id !== null))]
        )
      : null;
    const rows = await Promise.all(
      jobs.map(async (job) => ({
        ...(await jobDict(job)),
        ...fetchProgressFields(progressById.get(job.id) ?? null),
        username: usernameById ? (usernameById.get(job.userId ?? "") ?? null) : user.username,
      }))
    );
    return c.json({ jobs: rows, total, page: paging.page, limit: paging.limit });
  }
...
app.post("/api/agent/jobs/:jobId/artifacts", async (c) => {
  ...
  const key = artifactKey(jobId, artifactName);
  // 2026-09-24 配額納入 job 位元組：同名成品被覆寫時先扣掉舊的大小，計數器
  // 才不會把同一個檔算兩次。An overwrite frees the previous object's bytes.
  const previous = await c.env.STORE.head(key);
  await c.env.STORE.put(key, content);
  await queries.addJobArtifactBytes(c.env.DB, jobId, content.byteLength - (previous?.size ?? 0));
...
app.put("/api/agent/jobs/:jobId/artifacts/raw/:token", async (c) => {
  ...
  // 2026-09-24 配額納入 job 位元組：覆寫前先記下舊成品的大小（見 legacy
  // multipart 路由）。A failed PUT below deletes the key, so the previous
  // object's bytes are gone either way and get subtracted in both branches.
  const previous = await c.env.STORE.head(key);
  ...
  try {
    ...
    if (previous) await queries.addJobArtifactBytes(c.env.DB, jobId, -previous.size);
    return errorJson(c, 413, "jobs.artifact_too_large", "Upload exceeds the declared size.");
  }
  ...
  // The bytes streamed straight into R2, so the stored size comes from a
  // HEAD (the declared `row.size` is what the agent CLAIMED; the object is
  // what it wrote).
  const stored = await c.env.STORE.head(key);
  await queries.addJobArtifactBytes(c.env.DB, jobId, (stored?.size ?? 0) - (previous?.size ?? 0));
...
app.post("/api/agent/jobs/:jobId/artifacts/confirm", async (c) => {
  ...
  // 2026-09-24 配額納入 job 位元組：這條路 Worker 沒經手位元組，所以只能用
  // HEAD 回來的大小記帳。同一個檔名第二次 confirm（agent 重傳）時舊物件早已
  // 被 R2 直傳覆寫、大小不可考，就把前一次的紀錄視為 0 —— 只有已經在
  // `result_hashes` 裡的名字才是「上一次確認過」的；那種情況下不再累加，
  // 而是靠檔案頁列表時的 lazy backfill 把整張 job 校正回真實值。
  // A repeat confirm for a name already recorded is NOT added again (the
  // overwritten object's size is unknowable here); `GET /api/me/artifacts`'s
  // backfill re-syncs the whole job from an R2 listing.
  if (!(filename in job.resultHashes)) {
    await queries.addJobArtifactBytes(c.env.DB, jobId, head.size);
  }
...
// --- 2026-09-20 檔案頁 §3：成品清單與刪除 ---------------------------------

/** `result_files` 的防禦式讀法：只留字串元素，其餘（舊資料、手寫 INSERT 塞
 * 進來的怪東西）一律丟掉 —— ports the former Python `_result_files_list`. */
function resultFilesList(job: Job): string[] {
  return Array.isArray(job.resultFiles) ? job.resultFiles.filter((n): n is string => typeof n === "string") : [];
}

/** 兩條刪除路由共用的「本人或 admin」＋「已終局」閘門。
 *
 * 404 `jobs.not_found` 與其他每一條 per-job 路由一樣（不洩漏存在與否），外加
 * 一個 409 `jobs.not_finished`：一張還在飛的單（queued/assigned/running）的成
 * 品正是 worker 即將上傳的那些，在 `job_done` 底下把檔名從 `result_files` 抽
 * 掉，會弄壞那張單自己正在寫的清單。回傳值是 `{job}` 或已經成形的錯誤
 * Response，呼叫端一律 `if ("response" in gate) return gate.response;`。
 * Ports the former Python `_job_for_artifact_delete`. */
async function jobForArtifactDelete(
  c: Context<{ Bindings: Env }>,
  jobId: string
): Promise<{ job: Job } | { response: Response }> {
  const job = await queries.getJobById(c.env.DB, jobId);
  if (!job) return { response: errorJson(c, 404, "jobs.not_found", "Job not found.") };
  if (!isOwnerOrAdmin(job, c.get(SESSION_VAR).user)) {
    return { response: errorJson(c, 404, "jobs.not_found", "Job not found.") };
  }
  if (job.status !== "done" && job.status !== "failed" && job.status !== "cancelled") {
    return {
      response: errorJson(c, 409, "jobs.not_finished", `Job is ${job.status}; artifacts can't be deleted yet.`),
    };
  }
  return { job };
}

/** R2 上一張 job 的成品檔名 -> 位元組數。一次 `list` 把整個
 * `artifacts/<job_id>/` 前綴抽乾（R2 的一頁上限 1000，超過就跟著 `cursor`
 * 續抽），取代 server 端那邊一個檔一次 `os.stat`：在 R2 上那會變成每個檔一
 * 次網路往返。 */
async function artifactSizes(store: R2Bucket, jobId: string): Promise<Map<string, number>> {
...
/** Delete ONE of a finished job's artifacts (2026-09-20 檔案頁 §3.2).
 *
 * The bytes go and the name leaves `result_files`; `result_hashes`, the
 * receipt, and the job row itself all stay. That asymmetry is deliberate --
 * the artifacts are the user's own storage to reclaim, but the ledger is
 * what the federation settles payments against, and a user must not be able
 * to erase evidence of work a worker did for them by deleting the output. */
app.delete("/api/jobs/:jobId/artifacts/:filename", requireCsrfUser, async (c) => {
  const jobId = c.req.param("jobId");
  const filename = c.req.param("filename");

  const gate = await jobForArtifactDelete(c, jobId);
  if ("response" in gate) return gate.response;

  const names = resultFilesList(gate.job);
  if (!names.includes(filename)) return errorJson(c, 404, "jobs.artifact_not_found", "Artifact not found.");

  let key: string;
  try {
    key = artifactKey(jobId, filename);
  } catch {
    // 一個儲存層根本不肯變成 key 的名字（穿越、保留裝置名）不可能有位元組
    // 在上面 -- 同下載路由的處理。
    return errorJson(c, 404, "jobs.artifact_not_found", "Artifact not found.");
  }
  // R2 的 delete 對不存在的 key 就是成功，與 `LocalStore.delete` 吞掉
  // FileNotFoundError 同一個語意：刪除是冪等的。
  // 2026-09-24 配額納入 job 位元組：刪之前先 HEAD 拿大小，計數器才扣得回來；
  // 位元組本來就不在的名字扣 0。
  const existing = await c.env.STORE.head(key);
  await c.env.STORE.delete(key);
  if (existing) await queries.addJobArtifactBytes(c.env.DB, jobId, -existing.size);

  const remaining = names.filter((n) => n !== filename);
  await queries.updateJobResultFiles(c.env.DB, jobId, remaining);
  return c.json({ ok: true, result_files: remaining });
});

/** Delete EVERY artifact of a finished job -- the 檔案頁's folder-level
 * 「刪除整批」. Same rules as the single-file route, applied to each name; a
 * name whose bytes are already gone still counts as deleted, so the list
 * always ends up empty. */
app.delete("/api/jobs/:jobId/artifacts", requireCsrfUser, async (c) => {
  const jobId = c.req.param("jobId");

  const gate = await jobForArtifactDelete(c, jobId);
  if ("response" in gate) return gate.response;

  for (const name of resultFilesList(gate.job)) {
    let key: string;
    try {
  ...
  // 2026-09-24 配額納入 job 位元組：整批刪光就是確定為 0（不是 NULL —— 這張
  // 單的成品狀態現在是已知的）。
  await queries.setJobArtifactBytes(c.env.DB, jobId, 0);
  await queries.updateJobResultFiles(c.env.DB, jobId, []);
  return c.json({ ok: true, result_files: [] });
});

/** Every artifact the CALLER owns, newest job first (檔案頁 §3.1).
 *
 * Personal by default. 2026-09-21 管理視角：an admin may send `?scope=all`
 * to list EVERY user's artifacts (each file then also carries `user_id` and
 * `username`); a non-admin sending it gets 403 `auth.forbidden`.
 *
 * A split parent has no `result_files` of its own -- the bytes live under
 * its children, which inherit both `user_id` and `label` -- so the children
 * list naturally and show the parent's name.
 *
 * What the STORE holds decides what is listed: a name in `result_files`
 * whose bytes are missing is skipped rather than offered as a broken
 * download. */
app.get("/api/me/artifacts", requireUser, async (c) => {
  const user = c.get(SESSION_VAR).user;
  const scopeAll = c.req.query("scope") === "all";
  if (scopeAll && user.role !== "admin") {
    return errorJson(c, 403, "auth.forbidden", "需要管理員權限。 / Admin role required.");
  }
  // 2026-09-21 分頁：以 job 為單位（見 `listDoneJobsWithResultsForUserPage`）。
  // 沒帶 `?page=` 維持原本整包 `{ files }`。
  const paging = parsePaging(c.req.query("page"), c.req.query("limit"));
  let jobs: Job[];
  let totalJobs = 0;
  if (paging) {
    const pageResult = await queries.listDoneJobsWithResultsForUserPage(c.env.DB, scopeAll ? null : user.uid, paging);
    jobs = pageResult.jobs;
    totalJobs = pageResult.total;
  } else if (scopeAll) {
    jobs = await queries.listDoneJobsWithResultsAll(c.env.DB);
  } else {
    jobs = await queries.listDoneJobsWithResultsForUser(c.env.DB, user.uid);
  }
  const usernameById = scopeAll
    ? await queries.getUsernamesByIds(
        c.env.DB,
        [...new Set(jobs.map((j) => j.userId).filter((id): id is string => id !== null))]
      )
  ...
  for (const job of jobs) {
    const names = resultFilesList(job).slice().sort();
    if (names.length === 0) continue;
    const sizes = await artifactSizes(c.env.STORE, job.id);
    // 2026-09-24 配額納入 job 位元組的 lazy backfill：0017 之前的單
    // `artifact_bytes` 是 NULL，這裡反正已經 list 過 R2，順手把加總寫回去，
    // 從此這張單就計入配額。只補 NULL 的，已知值不動（它由上傳／刪除路徑維
    // 護，一次 R2 list 與計數器之間的短暫差異不該互相覆蓋）。
    if (job.artifactBytes === null) {
      await queries.setJobArtifactBytes(c.env.DB, job.id, [...sizes.values()].reduce((sum, n) => sum + n, 0));
    }
    for (const name of names) {
      ...
      files.push({
        ...
        ...(usernameById
          ? { user_id: job.userId, username: usernameById.get(job.userId ?? "") ?? null }
          : {}),
      });
    }
  }
  if (paging) return c.json({ files, total_jobs: totalJobs, page: paging.page, limit: paging.limit });
  return c.json({ files });
});
