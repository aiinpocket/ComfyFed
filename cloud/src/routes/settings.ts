/**
 * `/api/settings` GET/POST -- ported from the former Python server (2026-09)
 ...
 * the complete set -- the former Python settings router defined no others; `admin_password_hash` and `session_secret` are internal
...
import { NSFW_CHECK_API_KEY_KEY, readApiKey } from "../core/nsfw_gate";
import {
...
const BAD_NSFW_KEY =
  "API key 看起來不對（不能含空白，最長 512 字元）。" +
  " / That API key does not look right (no whitespace, at most 512 characters).";
...
interface CurrentSettings {
  ...
  /** 2026-09-20 NSFW 閘：明文永遠不出門，console 只需要知道「有沒有填」。 */
...
async function currentSettings(db: D1Database): Promise<CurrentSettings> {
  ...
  return {
    ...
    nsfw_check_api_key_set: (await readApiKey(db)) !== "",
  };
}
...
app.post("/api/settings", requireCsrf, async (c) => {
  ...
  if (body.nsfw_check_api_key !== undefined && body.nsfw_check_api_key !== null) {
    // 只擋明顯貼錯的東西（含空白／換行、或長到不像 key）；不驗證前綴，免得
    // Anthropic 換格式時 admin 填不進去。空字串 = 清掉、只跑規則層。
    if (typeof body.nsfw_check_api_key !== "string") {
      return errorJson(c, 400, "settings.bad_nsfw_check_api_key", BAD_NSFW_KEY);
    }
    const key = body.nsfw_check_api_key.trim();
    if (key.length > 512 || /\s/.test(key)) {
      return errorJson(c, 400, "settings.bad_nsfw_check_api_key", BAD_NSFW_KEY);
    }
    updates[NSFW_CHECK_API_KEY_KEY] = key;
  }
