/**
 * `/api/setup/*` and `/api/auth/*` -- ported from the former Python server
 * (2026-09); this file is now the only implementation.
 *
 * Exact routes mirrored from the former Python server: POST /api/auth/login, POST
...
// enumerate valid usernames. Mirrors the former Python module-level `_DUMMY_HASH`
 ...
 * token -- mirrors the former Python `issue_session_cookie`. Payload is `{uid, role,
...
app.post("/api/auth/login", async (c) => {
  ...
  // 422 envelope (the former Python server's app-wide validation-error handler) -- cloud
  ...
  // server's existing, load-bearing convention (coverage originally ported from the
  // Python suite's `test_validation_error_uses_the_standard_error_envelope`).
...
// Copied verbatim from the former Python token routes (bilingual there, so bilingual
