 ...
 * installer endpoints. Ported from the former Python server (2026-09); this
 * file is now the only implementation -- see the
 ...
 * (`cloud/scripts/build.mjs`) from `cloud/packaged/installers/` into
...
// substituted into. Ported verbatim from the former Python `_TOKEN_RE`.
...
// the former Python `_PLATFORM_URL_RE`: scheme, host (letters/digits/
...
/** Fixed, typed error envelope -- matches the former Python
 * `_error_response` shape EXACTLY (`{error, message}`, flat -- NOT the
 * `{error: {code, message}}` envelope `lib/guard.ts`'s `errorJson` uses
 * elsewhere in this codebase, since the coverage originally ported from
 * the Python suite asserts this exact flat shape). */
...
async function renderScript(env: Env, filename: string, requestUrl: string, tokenParam: string | undefined): Promise<string> {
  ...
  // the former Python `_render` order.
