/**
 * `GET /metrics` -- Prometheus text exposition, ported from the former Python
 * server (2026-09): its `metrics` module (metric names/help/type/buckets) +
 * its `/metrics` app route (the `metrics_public` gate). Hand-rendered
 ...
 * call sites scattered across its agentws/dispatch modules (worker connect/
 ...
 *    `mark_failed` in the former Python dispatcher) into a process-lifetime histogram. We
 ...
 *    verbatim from the former Python server).
 ...
 *    live event existed in Python, the former agentws handshake handler calling
...
// Copied verbatim from the former Python `_JOB_WAIT_BUCKETS`/`_JOB_RUN_BUCKETS`.
...
/** Bucket bounds are declared as ints in the former Python server but prometheus_client
 ...
 * the former Python `is_public`: defaults to true (public) when the
...
app.get("/metrics", async (c) => {
  if (!(await isPublic(c.env.DB))) {
    // Final review finding #2: admin-only, matching the former Python
    // server (`user is None or user.role != "admin"`) -- a plain
