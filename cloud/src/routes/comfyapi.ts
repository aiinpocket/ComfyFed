 ...
 * frontend can drive the federation. Ported from the former Python server
 * (2026-09); this file is now the only implementation, including the
 * deliberate deviations from upstream ComfyUI it ports
 ...
 * `/api/jobs`, not here). Nothing in
 * the former Python router added `require_csrf` anywhere, `POST /prompt` included: the
...
import { deriveLabel } from "../core/label";
 ...
 * "<zh-TW> / <English>"}` -- ports the former Python `_model_fetch_error`. Flat
...
// ports the former Python `_object_info_cache`, module-level (per-isolate) here
...
app.post("/comfy/api/prompt", async (c) => {
  ...
  // 2026-09-20 NSFW 閘：和 console 的 `POST /api/jobs` 同一個判定，只是用面板
  // 看得懂的 error envelope 回。放在缺模型檢查之前 -- 一個不准做 NSFW 的人不
  // 需要先被告知該去哪裡下載那個模型。`classify` 走模組命名空間（見
  // core/nsfw_gate.ts 檔頭）。
  const nsfwRefused = await nsfwGate.checkSubmission(c.env, user.uid, promptObj, {
    classify: nsfwGate.classifyWithClaude,
  });
  if (nsfwRefused !== null) {
    return comfyError("comfyfed." + nsfwGate.NSFW_NOT_ALLOWED_CODE, "NSFW review / NSFW 審核", nsfwRefused);
  }
  ...
  await queries.insertJob(c.env.DB, {
    ...
    userId: user.uid,
    // 2026-09-20 檔案頁 §2：面板沒有地方填名稱，只能從 workflow 的第一個
    // `Save*` 節點推；推不出來就存 NULL。
    label: deriveLabel(promptObj),
  ...
  // 2026-09-24 配額納入 job 位元組：面板送件複製到 `job_inputs/<job_id>/`
  // 的位元組也要記到 `input_bytes`，否則同一個檔在 staging 刪掉之後就從配額
  // 消失了。Same counter `createJobFromWorkflow` writes (routes/jobs.ts).
  await queries.setJobInputBytes(c.env.DB, jobId, inputBytes);
...
app.post("/comfy/api/interrupt", async (c) => {
  ...
  const job = running[0];
  if (job) {
    // Fire-and-forget, matching the former Python /interrupt, which
...
app.get("/comfy/api/view", async (c) => {
  ...
  if (subfolder) {
    ...
    // n2 (final review): two-key sort matching the former Python `ORDER BY
...
//
// Ports the former Python `/userdata` section, answering the pinned
// frontend's exact call shapes (`listUserDataFullInfo`, `getUserData`,
// `storeUserData`, `deleteUserData`, `moveUserData`); as there,
// a listing's `path` is relative to the REQUESTED `dir` rather than to
...
// and how job artifacts/inputs are counted (2026-09-24: they ARE part of the
// quota now, via the per-job counters).
 ...
 * replacement. Mirrors the former Python `_origin_rejected`. */
...
app.post("/comfy/api/userdata/:src{.+}/move/:dest{.+}", async (c) => {
  ...
  const moveLimits = await readLimits(c.env.DB, user.uid);
...
app.post("/comfy/api/userdata/:path{.+}", async (c) => {
  ...
  // of it, mirroring the former Python server's identical short-circuit. The post-read
  ...
  const limits = await readLimits(c.env.DB, user.uid);
  ...
  // writes succeed. the former Python server answered 409 `conflict` for that same pair
...
app.post("/comfy/api/comfyfed/model-fetch", async (c) => {
  ...
  try {
    result = await modelFetch.createFetchJob(
      ...
      // once at import: the former Python route likewise left `head` unset so
