...
// Ports the highest-value cases from the original Python suite: the
// recursive strip (per-node, top-level, and subgraph shapes -- the exact
// fixture workflows from that suite, copied verbatim below rather than as
// separate JSON files, mirroring how the Python suite itself kept them as
...
// --- Fixture workflows, copied verbatim from the original Python suite -----
...
describe("auth gating -- session gate covers every /comfy/templates/* shape (review round 1, m2)", () => {
  ...
  // mirroring the former Python `_comfy_session_gate`) that now runs BEFORE these
...
// ---------------------------------------------------------------------------
// "我的範本 / My templates" -- parity with the original Python suite's
...
// ---------------------------------------------------------------------------
// "共用庫 / Shared library" -- ports the same-named block of
// the original Python suite: every admin's folder is published to all,
// un-prefixed, ahead of packaged/official; ordinary users stay private.

const SHARED_WORKFLOW = { version: 0.4, nodes: [{ id: 1, type: "Note" }], links: [], groups: [] };

describe("共用庫 / Shared library (admin templates)", () => {
  it("publishes an admin's template into the ComfyFed category for every user, un-prefixed", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const alice = await userSession("alice");
    const bob = await userSession("bob");
    await putMyTemplate(await uidOf("admin"), "studio-portrait", { thumbnail: ".webp", workflow: SHARED_WORKFLOW });

    for (const who of [alice, bob]) {
      const index = (await call("/comfy/templates/index.json", { cookie: who.cookie })).body as any[];
      expect(titlesOf(index)).toEqual(["ComfyFed"]);
      expect(index[0].templates).toEqual([
        ...COMFYFED_CATEGORY.templates,
        { name: "studio-portrait", title: "studio-portrait", description: "", mediaType: "image", mediaSubtype: "webp" },
      ]);

      const r = await call("/comfy/templates/studio-portrait.json", { cookie: who.cookie });
      expect(r.status).toBe(200);
      expect(r.body).toEqual(SHARED_WORKFLOW);
      const media = await rawGet("/comfy/templates/studio-portrait-1.webp", who.cookie);
      expect(media.status).toBe(200);
      expect(media.headers.get("content-type")).toBe("image/webp");
    }
  });

  it("lets an admin's same-named file replace the packaged workflow while keeping its card", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    await putPackagedJson("comfyfed-wuxia-t2i.json", MY_WORKFLOW);
    const alice = await userSession("alice");
    await putMyTemplate(await uidOf("admin"), "comfyfed-wuxia-t2i", { workflow: SHARED_WORKFLOW });

    const index = (await call("/comfy/templates/index.json", { cookie: alice.cookie })).body as any[];
    // No duplicate entry, and the packaged card (title etc.) is untouched.
    expect(index[0].templates).toEqual(COMFYFED_CATEGORY.templates);

    const r = await call("/comfy/templates/comfyfed-wuxia-t2i.json", { cookie: alice.cookie });
    expect(r.body).toEqual(SHARED_WORKFLOW);
  });

  it("gives the admin no private category and never resolves my_ for them", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const admin = await loginSession();
    await putMyTemplate(await uidOf("admin"), "admin-thing");

    const index = (await call("/comfy/templates/index.json", { cookie: admin.cookie })).body as any[];
    expect(titlesOf(index)).toEqual(["ComfyFed"]);
    expect(index[0].templates.at(-1).name).toBe("admin-thing");
    expect((await call("/comfy/templates/admin-thing.json", { cookie: admin.cookie })).status).toBe(200);
    expect((await call("/comfy/templates/my_admin-thing.json", { cookie: admin.cookie })).status).toBe(404);
  });

  it("keeps a user's save of a shared template private to them", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const alice = await userSession("alice");
    const bob = await userSession("bob");
    await putMyTemplate(await uidOf("admin"), "studio-portrait", { workflow: SHARED_WORKFLOW });
    const aliceVersion = { ...MY_WORKFLOW, extra: { by: "alice" } };
    await putMyTemplate(await uidOf("alice"), "studio-portrait", { workflow: aliceVersion });

    const index = (await call("/comfy/templates/index.json", { cookie: alice.cookie })).body as any[];
    expect(titlesOf(index)).toEqual([MY_TITLE, "ComfyFed"]);
    expect(index[0].templates.map((t: any) => t.name)).toEqual(["my_studio-portrait"]);
    expect(index[1].templates.at(-1).name).toBe("studio-portrait");
    expect((await call("/comfy/templates/my_studio-portrait.json", { cookie: alice.cookie })).body).toEqual(aliceVersion);
    expect((await call("/comfy/templates/studio-portrait.json", { cookie: alice.cookie })).body).toEqual(SHARED_WORKFLOW);

    const bobIndex = (await call("/comfy/templates/index.json", { cookie: bob.cookie })).body as any[];
    expect(titlesOf(bobIndex)).toEqual(["ComfyFed"]);
    expect((await call("/comfy/templates/my_studio-portrait.json", { cookie: bob.cookie })).status).toBe(404);
    expect((await call("/comfy/templates/studio-portrait.json", { cookie: bob.cookie })).body).toEqual(SHARED_WORKFLOW);
  });

  it("merges into the official and localized indexes, and beats an official file at fetch time", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    await putOfficialJson("index.json", [FLUX_CATEGORY]);
    await putOfficialJson("index.zh.json", [{ ...FLUX_CATEGORY, title: "Flux (zh)" }]);
    await putOfficialJson("flux_dev.json", FLUX_WORKFLOW);
    const alice = await userSession("alice");
    await putMyTemplate(await uidOf("admin"), "shared-one");
    await putMyTemplate(await uidOf("admin"), "flux_dev", { workflow: SHARED_WORKFLOW });

    for (const [filename, officialTitle] of [
      ["index.json", "Flux"],
      ["index.zh.json", "Flux (zh)"],
    ] as const) {
      const index = (await call(`/comfy/templates/${filename}`, { cookie: alice.cookie })).body as any[];
      expect(titlesOf(index)).toEqual(["ComfyFed", officialTitle]);
      expect(index[0].templates.map((t: any) => t.name)).toEqual(["comfyfed-wuxia-t2i", "flux_dev", "shared-one"]);
      expect(index[1].templates).toEqual(FLUX_CATEGORY.templates);
    }

    const r = await call("/comfy/templates/flux_dev.json", { cookie: alice.cookie });
    expect(r.body).toEqual(SHARED_WORKFLOW); // raw admin graph, not the stripped official one
  });

  it("publishes only enabled admins' folders; a disabled admin and plain users do not publish", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const admin = await loginSession();
    const created = await call("/api/users", {
      json: { username: "admin2", role: "admin", password: "a-long-password1" },
      cookie: admin.cookie,
      headers: { "X-CSRF": admin.csrf },
    });
    expect(created.status).toBe(200);
    const admin2 = created.body.id as string;
    await call("/api/users", {
    ...
    await putMyTemplate(admin2, "from-admin2");
    await putMyTemplate(await uidOf("bob"), "from-bob");

    const namesFor = async (cookie: string | null) =>
      ((await call("/comfy/templates/index.json", { cookie })).body as any[])
        .find((c) => c.title === "ComfyFed")
        .templates.map((t: any) => t.name);

    expect(await namesFor(bobLogin.setCookie)).toEqual(["comfyfed-wuxia-t2i", "from-admin2"]);
    expect((await call("/comfy/templates/from-bob.json", { cookie: bobLogin.setCookie })).status).toBe(404);

    const patched = await call(`/api/users/${admin2}`, {
      method: "PATCH",
      json: { disabled: true },
      cookie: admin.cookie,
      headers: { "X-CSRF": admin.csrf },
    ...
    expect(await namesFor(bobLogin.setCookie)).toEqual(["comfyfed-wuxia-t2i"]);
    expect((await call("/comfy/templates/from-admin2.json", { cookie: bobLogin.setCookie })).status).toBe(404);
  });

  it("resolves a name collision between admins to the lowest uid, once", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const admin = await loginSession();
    const created = await call("/api/users", {
      json: { username: "admin2", role: "admin", password: "a-long-password1" },
    ...
    const uids = [await uidOf("admin"), created.body.id as string].sort();
    const first = { ...MY_WORKFLOW, extra: { owner: "first" } };
    const second = { ...MY_WORKFLOW, extra: { owner: "second" } };
    await putMyTemplate(uids[0]!, "clash", { workflow: first });
    await putMyTemplate(uids[1]!, "clash", { workflow: second });

    const alice = await userSession("alice");
    expect((await call("/comfy/templates/clash.json", { cookie: alice.cookie })).body).toEqual(first);
    const index = (await call("/comfy/templates/index.json", { cookie: alice.cookie })).body as any[];
    expect(index[0].templates.filter((t: any) => t.name === "clash")).toHaveLength(1);
  });

  it("does not leak the admin's entries into the shared merged-index cache", async () => {
    await putPackagedJson("index.json", [COMFYFED_CATEGORY]);
    const alice = await userSession("alice");
    await putMyTemplate(await uidOf("admin"), "transient");

    const first = (await call("/comfy/templates/index.json", { cookie: alice.cookie })).body as any[];
    expect(first[0].templates.at(-1).name).toBe("transient");

    await store().delete(`userdata/${await uidOf("admin")}/${MY_SUBDIR}/transient.json`);
    const second = (await call("/comfy/templates/index.json", { cookie: alice.cookie })).body as any[];
    expect(second[0].templates).toEqual(COMFYFED_CATEGORY.templates);
    expect((await call("/comfy/templates/transient.json", { cookie: alice.cookie })).status).toBe(404);
  });
});
