/**
 * 2026-09-19 配方（spec §5）測試：檔案格式、參數驗證、渲染與三個端點。
 *
 * 配方是 `import` 進 bundle 的 JSON（`src/core/recipes.ts`），壞掉的 JSON 在
 * build 期就爆，而不是 runtime 安靜地少一筆；「打包沒漏檔」由 `loadRecipes()`
 * 非空的斷言把關。
 */
...
// --- 2026-09-20 檔案頁 §2：配方跑出來的單要有名稱 ---
// 原 Python 測試套件的同名三個 test_run_*label* 案例。

describe("POST /api/recipes/{id}/run label (檔案頁 §2)", () => {
  it("labels the job with the recipe id", async () => {
    const session = await adminSession();
    const r = await run({ cookie: session.cookie, headers: { "X-CSRF": session.csrf } }, { prompt: "a fox" });
    expect(r.status).toBe(201);
    expect((await jobRow(r.body.job_id)).label).toBe("flux-t2i");
  });

  it("lets the body label override the recipe id", async () => {
    const session = await adminSession();
    const r = await call("/api/recipes/flux-t2i/run", {
      method: "POST",
      json: { params: { prompt: "a fox" }, label: "  狐狸練習  " },
      cookie: session.cookie,
      headers: { "X-CSRF": session.csrf },
    });
    expect(r.status).toBe(201);
    expect((await jobRow(r.body.job_id)).label).toBe("狐狸練習");
  });

  it("falls back to the derived label when the body label is blank", async () => {
    const session = await adminSession();
    const r = await call("/api/recipes/flux-t2i/run", {
      method: "POST",
      json: { params: { prompt: "a fox" }, label: "   " },
      cookie: session.cookie,
      headers: { "X-CSRF": session.csrf },
    });
    expect(r.status).toBe(201);
    // 全空白的名稱 normalize 成 null，於是 createJobFromWorkflow 從 workflow
    // 推 —— flux-t2i 的 SaveImage `filename_prefix` 是 "comfyfed_recipe"。
    expect((await jobRow(r.body.job_id)).label).toBe("comfyfed_recipe");
  });
});
