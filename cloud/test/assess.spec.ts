import { modelBackends, SOURCES, DEFAULT_MODEL_BACKENDS } from "../src/core/model_guide";
import { describe, expect, it } from "vitest";
import { modelNodes, mpsShimActive, verdict, partitionFleetFetchable, fleetWideGaps, exclusionKey, type JobNeeds } from "../src/core/assess";
import * as modelGuide from "../src/core/model_guide";
import type { Worker } from "../src/db/queries";

// Ports the `model_nodes` cases from the original Python suite -- `extract`/
...
// from the original Python suite's fetch-manifest section (`_worker` /
...
function makeWorker(id: string, overrides: Partial<Worker> = {}): Worker {
  return {
    ...
    hardware: {},
    dynamic: {},
    // "cuda" by default: since 2026-09-20 a model job is only eligible on an
    // NVIDIA worker, and nearly every test here hands the worker a model.
    backend: "cuda",
    torchVersion: "",
    nodeClasses: [],
    modelInventory: [],
...
// Ports the original Python suite's three `unverified_models` cases.
...
// value cases from the original Python suite's peer-only section.
...
import { signature, extract, estimateResidentGb, estimateVram, unifiedMemoryGb } from "../src/core/assess";
...
describe("signature (Phase 3.3 §2.1)", () => {
  ...
  it("treats an integral float like the same int (final-review M1)", async () => {
    ...
    // `Number.isInteger` 本來就是這個行為；原 Python 的 `_literal_int` 以前只收
    // `int`，兩棧因此對一張存成 `20.0` 的圖算出不同簽章。這是那個 parity
    // 的 TS 側釘子（原 Python 測試套件同名測試）。
    const asInt = {
      "1": { class_type: "KSampler", inputs: { steps: 20 } },
...
// --- 2026-09-20: model jobs are NVIDIA-only unless the guide says otherwise -

describe("backend_unsupported (model jobs are NVIDIA-only)", () => {
  const inv = [{ name: "flux1-dev.safetensors", directory: "unet" }] as any;
  const needsModel = () => extract({ "1": { class_type: "UNETLoader", inputs: { unet_name: "flux1-dev.safetensors" } } });

  it.each(["mps", "rocm", ""])("refuses a model job on a %j worker", (backend) => {
    const w = makeWorker("mac", { backend, nodeClasses: ["UNETLoader"], modelInventory: inv });
    const v = verdict(w, needsModel(), {}, [w]);
    expect(v.kind).toBe("ineligible");
    expect(v.reasons).toContain(`backend_unsupported:${backend || "unknown"}:flux1-dev.safetensors`);
  });

  it("accepts the same job on a cuda worker", () => {
    const w = makeWorker("nv", { backend: "cuda", nodeClasses: ["UNETLoader"], modelInventory: inv });
    expect(verdict(w, needsModel(), {}, [w]).kind).toBe("eligible");
  });

  it("still hands a zero-model job to a Mac", () => {
    const w = makeWorker("mac", { backend: "mps", nodeClasses: ["VideoConcat"] });
    const needs = extract({ "1": { class_type: "VideoConcat", inputs: {} } });
    expect(verdict(w, needs, {}, [w]).kind).toBe("eligible");
  });

  it("defaults an unlisted model to cuda only, and every curated entry includes cuda", () => {
    expect([...modelBackends("never-heard-of.safetensors")]).toEqual(["cuda"]);
    for (const source of Object.values(SOURCES)) {
      expect(source.backends ?? DEFAULT_MODEL_BACKENDS).toContain("cuda");
    }
  });
});


// --- 2026-09-23: a Mac running the agent's MPS shim takes model jobs -------

describe("mps_quant_compat (a Mac with the shim is treated like cuda)", () => {
  const inv = [{ name: "flux1-dev.safetensors", directory: "unet" }] as any;
  const needsModel = () => extract({ "1": { class_type: "UNETLoader", inputs: { unet_name: "flux1-dev.safetensors" } } });

  it("accepts a model job on an mps worker whose hardware says mps_quant_compat: true", () => {
    const w = makeWorker("mac", { backend: "mps", nodeClasses: ["UNETLoader"], modelInventory: inv, hardware: { mps_quant_compat: true } });
    const v = verdict(w, needsModel(), {}, [w]);
    expect(v.kind).toBe("eligible");
    expect(v.reasons.some((r) => r.startsWith("backend_unsupported"))).toBe(false);
  });

  it.each([false, "true", 1, null, undefined])("keeps refusing when the flag is %j", (flag) => {
    const w = makeWorker("mac", { backend: "mps", nodeClasses: ["UNETLoader"], modelInventory: inv, hardware: { mps_quant_compat: flag } });
    const v = verdict(w, needsModel(), {}, [w]);
    expect(v.kind).toBe("ineligible");
    expect(v.reasons).toContain("backend_unsupported:mps:flux1-dev.safetensors");
  });

  it("ignores the flag on a non-mps worker", () => {
    const w = makeWorker("box", { backend: "rocm", nodeClasses: ["UNETLoader"], modelInventory: inv, hardware: { mps_quant_compat: true } });
    expect(mpsShimActive(w)).toBe(false);
    expect(verdict(w, needsModel(), {}, [w]).kind).toBe("ineligible");
  });
});

// --- 2026-09-23 cross-backend §4: unified memory resident-set gate ---------
// Ports the section of the same name from the original Python suite.

describe("unified memory resident-set gate (2026-09-23 §4)", () => {
  const h3Needs = () =>
    extract({
      "1": { class_type: "UNETLoader", inputs: { unet_name: "h3_int8.safetensors" } },
      "2": { class_type: "CLIPLoader", inputs: { clip_name: "qwen32b.safetensors" } },
      "3": { class_type: "VAELoader", inputs: { vae_name: "h3_vae.safetensors" } },
    });
  const h3Inventory = () =>
    [
      { name: "diffusion_models/h3_int8.safetensors", directory: "diffusion_models", size: 20.0 },
      { name: "text_encoders/qwen32b.safetensors", directory: "text_encoders", size: 15.0 },
      { name: "vae/h3_vae.safetensors", directory: "vae", size: 5.0 },
    ] as any;
  const H3_NODES = ["UNETLoader", "CLIPLoader", "VAELoader"];

  it("estimateResidentGb sums every model", () => {
    const w = makeWorker("mac", { modelInventory: h3Inventory() });
    expect(estimateResidentGb(h3Needs().models, [w])).toBeCloseTo(40.0 * 1.15, 10);
    expect(estimateResidentGb(new Set(["unknown.safetensors"]), [w])).toBeNull();
  });

  it("refuses a model set that does not fit the usable pool", () => {
    const hw = { vram_gb: 48.0, ram_gb: 48.0, unified_memory: true, mps_quant_compat: true };
    const w = makeWorker("mac", { nodeClasses: H3_NODES, modelInventory: h3Inventory(), hardware: hw, backend: "mps" });
    const v = verdict(w, h3Needs(), {}, [w]);
    expect(v.kind).toBe("ineligible");
    expect(v.reasons).toContain("memory:46.0>40.8");
  });

  it("accepts a model set that fits and never double counts ram", () => {
    const hw = { vram_gb: 64.0, ram_gb: 64.0, unified_memory: true, mps_quant_compat: true };
    const w = makeWorker("mac", { nodeClasses: H3_NODES, modelInventory: h3Inventory(), hardware: hw, backend: "mps" });
    const v = verdict(w, h3Needs(), {}, [w]);
    expect(v.kind, v.reasons.join(",")).toBe("eligible");
    expect(v.reasons.some((r) => r.startsWith("memory:"))).toBe(false);
  });

  it("discrete gpu keeps the largest-model rule", () => {
    const hw = { vram_gb: 16.0, ram_gb: 64.0 };
    const w = makeWorker("nv", { nodeClasses: H3_NODES, modelInventory: h3Inventory(), hardware: hw, backend: "cuda" });
    const needs = h3Needs();
    needs.estVramGb = estimateVram(needs.models, [w]);
    const v = verdict(w, needs, {}, [w]);
    expect(v.kind).toBe("eligible");
    expect(v.warnings.some((r) => r.startsWith("vram_offload:"))).toBe(true);
    expect(v.reasons.some((r) => r.startsWith("memory:"))).toBe(false);
  });

  it("unifiedMemoryGb requires the flag and a positive number", () => {
    expect(unifiedMemoryGb({ vram_gb: 64.0, unified_memory: true })).toBe(64.0);
    expect(unifiedMemoryGb({ vram_gb: 64.0 })).toBeNull();
    expect(unifiedMemoryGb({ vram_gb: true, unified_memory: true })).toBeNull();
    expect(unifiedMemoryGb({ vram_gb: 0, unified_memory: true })).toBeNull();
  });
});
