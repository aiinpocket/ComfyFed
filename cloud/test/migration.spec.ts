...
describe("D1 migration 0014_job_label", () => {
  it("adds a nullable jobs.label with no default and no backfill", async () => {
    const db = (env as any).DB as D1Database;
    const cols = await db.prepare("PRAGMA table_info(jobs)").all<{ name: string; type: string; notnull: number; dflt_value: string | null }>();
    const label = cols.results.find((c) => c.name === "label");
    expect(label, "jobs.label missing").toBeTruthy();
    expect(label!.type).toBe("TEXT");
    // 可為 NULL 且沒有 DEFAULT：「沒有名稱」只有 NULL 一種表示法，舊列不回填。
    expect(label!.notnull).toBe(0);
    expect(label!.dflt_value).toBeNull();

    await db
      .prepare("INSERT INTO jobs (id, workflow_json, status, created_at) VALUES ('label-default', '{}', 'queued', '2026-09-20 00:00:00')")
      .run();
    const row = await db.prepare("SELECT label FROM jobs WHERE id = 'label-default'").first<{ label: string | null }>();
    expect(row!.label).toBeNull();
    await db.prepare("DELETE FROM jobs WHERE id = 'label-default'").run();
  });
});

describe("D1 migration 0017_job_bytes", () => {
  it("adds nullable jobs.artifact_bytes / jobs.input_bytes with no default and no backfill", async () => {
    const db = (env as any).DB as D1Database;
    const cols = await db.prepare("PRAGMA table_info(jobs)").all<{ name: string; type: string; notnull: number; dflt_value: string | null }>();
    for (const name of ["artifact_bytes", "input_bytes"]) {
      const col = cols.results.find((c) => c.name === name);
      expect(col, `jobs.${name} missing`).toBeTruthy();
      expect(col!.type).toBe("INTEGER");
      // 可為 NULL 且沒有 DEFAULT：NULL = 還不知道（舊列不回填），0 = 確定為空。
      expect(col!.notnull).toBe(0);
      expect(col!.dflt_value).toBeNull();
    }

    await db
      .prepare("INSERT INTO jobs (id, workflow_json, status, created_at) VALUES ('bytes-default', '{}', 'queued', '2026-09-24 00:00:00')")
      .run();
    const row = await db
      .prepare("SELECT artifact_bytes, input_bytes FROM jobs WHERE id = 'bytes-default'")
      .first<{ artifact_bytes: number | null; input_bytes: number | null }>();
    expect(row!.artifact_bytes).toBeNull();
    expect(row!.input_bytes).toBeNull();
    await db.prepare("DELETE FROM jobs WHERE id = 'bytes-default'").run();
  });
});
