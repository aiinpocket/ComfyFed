...
// Ports the highest-value cases from the original Python suite (grant
// issuance seeder selection/signing, peer-served atomic booking, error
// codes). The grant
...
async function makeSeeder(
  ...
  // pass null/0 explicitly. Parity: the original Python suite's `_make_online_seeder`.
  await db()
    .prepare(
...
// Ports the original Python suite's matching block.
