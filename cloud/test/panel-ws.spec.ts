...
// Ports the panel-facing half of the original Python suite against the
...
// exactly mirroring how the former Python agent-WS handlers called the
// panel-WS broadcast functions.
//
...
// Ports the original Python case
...
// (the former Python dispatch loop only ever consulted `agentws._connections`,
...
//
// JSON-shape parity with the original Python suite: every frame
...
describe("split families on the panel (§3.7)", () => {
  ...
  it("relays a stale child's requeue as the parent -- but only once the parent is back in the queue", async () => {
    // Mirrors the original Python case
