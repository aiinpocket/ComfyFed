...
// Ports the highest-value cases from the original Python suite.
// Fix round 1: a hash
...
// requires a platform-verified endpoint (parity: the original Python suite's
// `_make_online_seeder`).
...
describe("entries: peer-only entries", () => {
  ...
  it("an unverified seeder (peer_reachable NULL) does not count (Phase 3.4 §4.2)", async () => {
    ...
    // `peer_reachable = 1` half -- parity with the former Python
