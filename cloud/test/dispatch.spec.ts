...
// Ports the pure-dispatch-logic assertions of the original Python suite
...
async function makeWorker(
  ...
  await db()
    ...
    .bind(
      ...
      // "cuda" by default: since 2026-09-20 a model job is only eligible on
      // an NVIDIA worker, and most tests here hand the worker a model. The
      // light-job-to-Mac tests pass backend: "mps" explicitly.
      opts.backend ?? "cuda",
      opts.protocol ?? 1,
      opts.autoFetch ? 1 : 0,
...
// Ports the former Python `assign_jobs` fetch-tier docstring cases: tier 2
...
// --- 2026-09-23 cross-backend §3: wait-or-run-now holds --------------------
// Exercises the D1 side of the feature (`busyWorkerCandidates` reading the
// running job's `dispatch_info` / `started_at`, `recordHolds` writing the
// hold into the queued job) on top of the pure `scheduler.applyHolds` cases
// in scheduler.spec.ts.

describe("assignJobs (busyWorkerIds holds, 2026-09-23 §3)", () => {
  const FLUX = "flux1-dev.safetensors";
  const INVENTORY = [{ name: "diffusion_models/flux1-dev.safetensors", size: 6 }];

  async function makeSignedJob(opts: { models?: string[]; createdAt?: Date } = {}): Promise<string> {
    const id = uniqueId("j");
    await db()
      .prepare(
        `INSERT INTO jobs (id, workflow_json, status, created_at, signature, required_models)
         VALUES (?, '{}', 'queued', ?, 'sig', ?)`
      )
      .bind(id, toSqliteTimestamp(opts.createdAt ?? new Date()), JSON.stringify(opts.models ?? [FLUX]))
      .run();
    return id;
  }

  async function makeRunningJob(workerId: string, startedAt: Date, predictedSeconds: number): Promise<string> {
    const id = uniqueId("running");
    await db()
      .prepare(
        `INSERT INTO jobs (id, workflow_json, status, worker_id, created_at, started_at, signature, required_models, dispatch_info)
         VALUES (?, '{}', 'running', ?, ?, ?, 'sig', ?, ?)`
      )
      .bind(
        id,
        ...
        toSqliteTimestamp(startedAt),
        JSON.stringify([FLUX]),
        JSON.stringify({ predicted_seconds: predictedSeconds, basis: "signature", load_seconds: 0, fetch_seconds: 0, candidates: 1 })
      )
      .run();
    return id;
  }

  async function setStats(workerId: string, ewma: number): Promise<void> {
    await db()
      .prepare("INSERT INTO worker_job_stats (worker_id, signature, ewma_seconds, samples, updated_at) VALUES (?, 'sig', ?, 3, ?)")
      .bind(workerId, ewma, toSqliteTimestamp(new Date()))
      .run();
  }

  async function fleet(now: Date): Promise<{ mac: string; nv: string }> {
    const mac = await makeWorker("mac", {
      backend: "mps",
      hardware: { mps_quant_compat: true },
      dynamic: { free_vram_gb: 0 },
      nodeClasses: [],
      modelInventory: INVENTORY,
    });
    const nv = await makeWorker("nv", { backend: "cuda", dynamic: { free_vram_gb: 24 }, modelInventory: INVENTORY });
    await setStats(nv, 500);
    // nv 跑了 100 秒、預估 400 -> 剩 300；Mac 沒有統計列 -> mps 先驗 0.12，
    // 預估 500 / 0.12 ≈ 4167 秒 >> 等 nv 的 300 + 500 = 800。
    await makeRunningJob(nv, new Date(now.getTime() - 100_000), 400);
    return { mac, nv };
  }

  it("holds a job for a busy cuda worker instead of handing it to an idle Mac, and records why", async () => {
    const now = new Date();
    const { mac, nv } = await fleet(now);
    const jobId = await makeSignedJob();

    const assignments = await dispatch.assignJobs(db(), [mac], null, null, now, null, null, [nv]);
    expect(assignments).toEqual([]);

    const row = await getJobRow(jobId);
    expect(row.status).toBe("queued");
    const info = JSON.parse(row.dispatch_info);
    expect(info.held_for).toBe(nv);
    expect(info.held_for_name).toBe("nv");
    expect(info.wait_seconds).toBeCloseTo(800 + 0.001, 2); // 300 eta + 500 predicted + tiebreak
    // 4166.67 predicted (500 / 0.12) + 9 load (6 GB cold on the Mac) + tiebreak
    expect(info.run_now_seconds).toBeCloseTo(500 / scheduler.speedPrior("mps") + 9 + 0.001, 2);
  });

  it("dispatches to the Mac as before when no busy worker is passed", async () => {
    const now = new Date();
    const { mac } = await fleet(now);
    const jobId = await makeSignedJob();

    const assignments = await dispatch.assignJobs(db(), [mac], null, null, now);
    expect(assignments.map((a) => [a.workerId, a.job.id])).toEqual([[mac, jobId]]);
  });

  it("ignores a busy worker whose running job is overdue past OVERDUE_FACTOR", async () => {
    const now = new Date();
    const mac = await makeWorker("mac", {
      backend: "mps",
      hardware: { mps_quant_compat: true },
    ...
    const nv = await makeWorker("nv", { backend: "cuda", dynamic: { free_vram_gb: 24 }, modelInventory: INVENTORY });
    await setStats(nv, 500);
    await makeRunningJob(nv, new Date(now.getTime() - 1_000_000), 400);
    const jobId = await makeSignedJob();
  ...
  it("clears a stale held_for once the hold no longer applies", async () => {
    const now = new Date();
    const { mac, nv } = await fleet(now);
    const jobId = await makeSignedJob();
    await dispatch.assignJobs(db(), [mac], null, null, now, null, null, [nv]);
    expect(JSON.parse((await getJobRow(jobId)).dispatch_info).held_for).toBe(nv);

    // 下一個 tick：nv 不再忙、而唯一 idle 的 worker 沒有這個模型 -> job 留在
    // queued，但舊的 held 資訊要被清掉。
    const bare = await makeWorker("bare", { backend: "cuda", dynamic: { free_vram_gb: 24 } });
    const assignments = await dispatch.assignJobs(db(), [bare], null, null, now, null, null, []);
    expect(assignments).toEqual([]);
    const row = await getJobRow(jobId);
    expect(row.status).toBe("queued");
    expect(JSON.parse(row.dispatch_info)).toEqual({});
  });
});
