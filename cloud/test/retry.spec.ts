 ...
 * adapter -- coverage originally ported case for case from the Python suite.
 *
 * 上半是完全不碰 D1 的計次／門檻／訊息彙整（和原 Python 實作逐行對照）；下半是
...
describe("summarizeFinalError", () => {
  ...
  it("is byte-identical to the Python message shape", () => {
    // The former Python f-string, spelled out once so a whitespace/punctuation drift
...
describe("worker_task_failures adapter", () => {
  ...
  it("hides the free-text fields from a non-admin reader", async () => {
    // final review I1 -- ports the original Python case
