...
// Ports the core agent-WS assertions of the original Python suite against the
 ...
 *
 * Ports the original Python suite's `_exhaust_attempts`: every test in
 ...
 * the original Python suite's `_fetch_ready_worker` + `_hello_and_idle`. */
...
describe("handshake", () => {
  ...
  it("rejects a merely DISABLED worker with 4403, not 4401", async () => {
    ...
    // and never prunes the registration. Parity: the original Python case
  ...
  it("rejects a soft-deleted worker with 4401", async () => {
    ...
    // good. Ports the original Python suite's deleted-handshake test.
    const kp = KEYPAIRS[0]!;
...
describe("hello", () => {
  ...
  // 2026-09-23：hello 帶目前狀態；busy + job_id 才算 busy，其餘照舊 idle。
  it("records a hello that says busy with a job id as busy, and everything else as online", async () => {
    const kp = KEYPAIRS[0]!;
    const workerId = await makeWorker({ pubkeyHex: kp.pubkey_hex });
    const ws = await connectAgent(workerId, kp.seed_hex);
    const none = expectNoMessage(ws, 300);
    ws.send(JSON.stringify({ type: "hello", protocol: 5, backend: "mps", state: "busy", job_id: "job-1" }));
    await none;
    let row = await db().prepare("SELECT status FROM workers WHERE id = ?").bind(workerId).first<{ status: string }>();
    expect(row!.status).toBe("busy");

    const none2 = expectNoMessage(ws, 300);
    ws.send(JSON.stringify({ type: "hello", protocol: 5, backend: "mps", state: "busy" }));
    await none2;
    row = await db().prepare("SELECT status FROM workers WHERE id = ?").bind(workerId).first<{ status: string }>();
    expect(row!.status).toBe("online");

    const none3 = expectNoMessage(ws, 300);
    ws.send(JSON.stringify({ type: "hello", protocol: 5, backend: "mps", state: "paused" }));
    await none3;
    row = await db().prepare("SELECT status FROM workers WHERE id = ?").bind(workerId).first<{ status: string }>();
  ...
  // Phase 3.2 F1 fix -- ports the former Python _parse_max_fetch_gb test coverage.
  ...
  // Seeder upload cap -- ports the former Python _parse_peer_upload_min_mbps coverage.
  ...
  it("omits peer_upload_min_mbps when hello reports null (both caps unlimited) or garbage", async () => {
    ...
    // so can never become a TTL divisor. Parity: the former Python
  ...
  // Phase 3.1 P2P seeder advertisement -- ports the former Python _parse_peer_url
  // test coverage.
...
//
// Ports the original Python suite's split-cancel/derivation block. The
...
// the original Python suite through the real Hub DO.
...
// the four tests appended to the original Python suite.
...
// ports the two tests appended to the original Python suite.
...
// 2026-09-21 分頁：`/internal/fetch_progress_batch` —— 一頁 job 的 fetch 進度
// 一次問完，取代 `GET /api/jobs` 過去每一列各打一次 `/internal/fetch_progress`。
describe("POST /internal/fetch_progress_batch", () => {
  it("returns progress keyed by job id for the ids that have one, omitting the rest", async () => {
    const kp = KEYPAIRS[0]!;
    const workerId = await makeWorker({ pubkeyHex: kp.pubkey_hex });
    const fetching = await makeJob({ status: "assigned", workerId });
    const idle = await makeJob({ status: "queued" });
    const ws = await connectAgent(workerId, kp.seed_hex);
    ws.send(
      JSON.stringify({
        type: "heartbeat",
    ...
    const body = await waitFor(
      async () => {
        const res = await hub().fetch("http://hub.internal/internal/fetch_progress_batch", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ job_ids: [fetching, idle, "nope"] }),
        });
        const json = await res.json<any>();
        return json.progress[fetching] ? json : undefined;
      },
      { label: "batch progress reflects the heartbeat" }
    );
    expect(body.progress).toEqual({
      [fetching]: { stage: "fetching_models", fetch_pct: 0.25, fetch_model: "m.safetensors" },
    });
    ws.close();
  });

  it("answers an empty map for an empty or malformed body", async () => {
    const res = await hub().fetch("http://hub.internal/internal/fetch_progress_batch", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "not json",
