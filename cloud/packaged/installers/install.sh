...
if [ -f "$MANAGED_MARKER" ] && [ "$MARKER_IS_DETECTED" -eq 0 ]; then
    ...
    if DETECTED_COMFY_URL="$("$VENV_PYTHON" "$HELPER_SCRIPT" check)"; then
        ...
        if [ "$OS_KIND" = "darwin" ]; then
            # 2026-09-23: fp8/int8/nvfp4 weights have no MPS kernels; the
            # agent ships a custom node that shims them (mps_compat.py).
            # Placed BEFORE the first start so it is live from day one.
            bilingual "安裝 Apple MPS 量化相容 shim..." "Installing the Apple MPS quantization shim..."
            "$VENV_PYTHON" -m comfyfed_agent.mps_compat "$COMFY_DIR" ||
                bilingual "MPS shim 安裝失敗（Mac 將只能接零模型工作）" "MPS shim install failed (this Mac will only get zero-model jobs)"
        fi
...
# ---------------------------------------------------------------------------
# 4a. Apple MPS quantization shim (macOS only; see agent/comfyfed_agent/mps_compat.py)
# ---------------------------------------------------------------------------
# Upgrades and detected (non-managed) ComfyUI installs: derive the ComfyUI dir
# from the configured models_dir and (re)write the shim. The running ComfyUI
# only picks it up after a restart -- the managed plist reload in section 5
# does that; a hand-run ComfyUI has to be restarted by its owner. The agent
# reports `mps_quant_compat` on every hello, so the platform hands this Mac
# model jobs from the first hello after the restart, and not before.
if [ "$OS_KIND" = "darwin" ] && [ -f "$AGENT_CONFIG_PATH" ]; then
    SHIM_COMFY_DIR="$("$VENV_PYTHON" - "$AGENT_CONFIG_PATH" <<'PYEOF'
import json, sys
try:
    cfg = json.load(open(sys.argv[1], encoding="utf-8"))
    from comfyfed_agent import mps_compat
    print(mps_compat.comfy_dir_from_models_dir(cfg.get("models_dir")) or "")
except Exception:
    print("")
PYEOF
)"
    if [ -n "$SHIM_COMFY_DIR" ]; then
        "$VENV_PYTHON" -m comfyfed_agent.mps_compat "$SHIM_COMFY_DIR" ||
            bilingual "MPS shim 安裝失敗（Mac 將只能接零模型工作）" "MPS shim install failed (this Mac will only get zero-model jobs)"
        if [ "$COMFY_MANAGED" -ne 1 ]; then
            bilingual "提示: 請重啟你自己的 ComfyUI，MPS shim 才會生效" \
                "Tip: restart your own ComfyUI so the MPS shim takes effect"
        fi
    fi
fi
...
UNITEOF
    ...
    if [ "$COMFY_MANAGED" -eq 1 ]; then
        ...
        # 2026-09-23: unload alone leaves a ComfyUI that was started by hand
        # (or by the fresh-install nohup above) running WITHOUT the MPS
        # shim, and the launcher's probe-first rule would then never start a
        # new one. Stop the managed ComfyUI explicitly so the load below
        # brings up one that has the shim.
        pkill -f "$COMFY_DIR/main.py" 2>/dev/null || true
        sleep 2
