...
- **免費方案下單一產出檔 ≤ 100MB**（Cloudflare Workers 請求體大小上限）。預設的 `direct` 上傳模式（agent → Worker → R2）受這個限制。
  - 解法：設定 `R2_S3_*` 四個 secrets（`R2_S3_ACCOUNT_ID`／`R2_S3_ACCESS_KEY_ID`／`R2_S3_SECRET_ACCESS_KEY`／`R2_S3_BUCKET`，從 Cloudflare Dashboard → R2 → Manage API Tokens 取得），四個都設了之後 `/api/agent/jobs/{id}/artifacts/presign` 會自動改回傳 S3 相容的預簽章 URL，agent 直接 PUT 進 R2，完全繞過 Worker 的請求體限制。
...
- **100MB per artifact on the free plan** (Cloudflare Workers' request body size cap). The default `direct` upload mode (agent → Worker → R2) is subject to this.
