 ...
 *
 * Ported from the former Python server's `official_templates.fetch()`
 * (2026-09); this script is now the only implementation. The PyPI
 * split-package rationale:
 ...
 *
 * Divergence from the former Python implementation, deliberate:
 *
 ...
 *   ADDED here, beyond what the Python version did (whole-wheel sha256
...
// Same cap as the former Python `_MAX_WHEEL_BYTES` -- generous
 ...
 * the partial file) the moment `maxBytes` is exceeded -- ports the former
 * Python `_download` streaming-to-disk + mid-stream cap
