 ...
 *
 *   (e) **Agent wheel** (`assets/agent/`) -- `build-wheel.mjs` packages
 *       `agent/comfyfed_agent/` into `comfyfed-<version>-py3-none-any.whl`
 *       plus a `release.json` manifest; `src/core/agent_release.ts`
 *       publishes it at runtime when it is newer than the stored release.
 */
...
import { writeAgentRelease } from "./build-wheel.mjs";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const CLOUD_DIR = path.join(HERE, "..");
const WEB_DIR = path.join(CLOUD_DIR, "..", "web");
const TEMPLATES_DATA_DIR = path.join(CLOUD_DIR, "packaged", "templates");
const INSTALLERS_SRC_DIR = path.join(CLOUD_DIR, "packaged", "installers");
...
const AGENT_RELEASE_ASSETS_DIR = path.join(ASSETS_DIR, "agent");
...
async function main() {
  ...
  // (e) the agent wheel + release.json -- see build-wheel.mjs and
  // src/core/agent_release.ts: every deploy is an agent release.
  await writeAgentRelease(AGENT_RELEASE_ASSETS_DIR);
