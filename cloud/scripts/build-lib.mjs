 ...
 * the pinned `comfyui-frontend-package` wheel (build.mjs's own
 * `FRONTEND_VERSION`/`FRONTEND_SHA256` constants are now the ONLY pin --
 * the former Python server's `comfy_frontend.py` twin is gone, so bump
 * them together there), extract everything under its
 ...
 * dropping them shrinks the asset upload for free -- the former Python
 * server's `extract_static` did NOT do this, since disk space was not the
 * same constraint for a locally-run server process).
 */
 ...
 * result has no `index.html` at its root -- same sanity check the former
 * Python `extract_static` made ("is this the right package?").
 *
